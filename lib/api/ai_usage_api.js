...
function configure(app, wares, ctx) {
  ...
  api.get('/monthly_summary', ctx.authorization.isPermitted('api:treatments:read'), async (req, res) => {
    try {
      ...
      let cost_input = req.settings.ai_llm_1k_token_costs_input;
      let cost_output = req.settings.ai_llm_1k_token_costs_output;

      // Validate and provide defaults if settings are invalid
      if (typeof cost_input !== 'number' || isNaN(cost_input)) {
        console.warn(`Invalid AI_LLM_1K_TOKEN_COSTS_INPUT setting: '${cost_input}'. Falling back to default.`);
        cost_input = 0.005; // Default value
      }
      if (typeof cost_output !== 'number' || isNaN(cost_output)) {
        console.warn(`Invalid AI_LLM_1K_TOKEN_COSTS_OUTPUT setting: '${cost_output}'. Falling back to default.`);
        cost_output = 0.015; // Default value
      }
