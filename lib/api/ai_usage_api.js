...
function configure(app, wares, ctx) {
  ...
  api.get('/monthly_summary', ctx.authorization.isPermitted('api:treatments:read'), async (req, res) => {
    try {
      const usageCollection = ctx.store.collection(USAGE_COLLECTION_NAME);
      const costInput = ctx.env.ai_llm_1k_token_costs_input;
      const costOutput = ctx.env.ai_llm_1k_token_costs_output;
      ...
      const monthlyPipeline = [
        ...
        },
        {
          $project: {
            ...
            total_cost: {
              $add: [
                { $multiply: [ { $divide: ["$total_prompt_tokens", 1000] }, costInput ] },
                { $multiply: [ { $divide: ["$total_completion_tokens", 1000] }, costOutput ] }
              ]
            },
            avg_cost_per_request: {
              $cond: {
                if: { $eq: ["$requests", 0] }, then: 0,
                else: {
                  $divide: [
                    {
                      $add: [
            ...
            avg_cost_per_day: {
              $cond: {
                if: { $eq: ["$total_days_requested", 0] }, then: 0,
                else: {
                  $divide: [
                    {
                      $add: [
