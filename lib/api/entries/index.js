...
const regexGuard = require('../../server/regex-guard');
const { mapSeries } = require('../../array-map');
...
function configure (app, wares, ctx, env) {
  ...
  function format_entries (req, res) {
    ...
    // The general pattern here is to reformat the data correctly into the
    // desired representation, enforcing basic rules (such as ensuring a `type`
    // property exists) via force_typed_data while mapping each element.
    return res.format({
      'text/plain': function() {
        mapSeries(res.entries || [], force_typed_data(type_params), function(err, out) {
  ...
  function insert_entries (req, res, next) {
    ...
    function persist (fn) {
      if (req.persist_entries) {
        // store everything
        return entries.persist(incoming, fn);
      }
      // support a preview mode, just lint everything (the lint step is a no-op)
      return fn(null, incoming);
    }
    ...
    // send everything to persistent storage
  ...
  function delete_records (req, res, next) {
    ...
    req.model.remove(query, function(err, stat) {
      ...
      // Normalize MongoDB result for backward compatibility with v3 driver
      // Frontend expects .n property for deleted count
      if (stat) {
        if (stat.n === undefined && stat.deletedCount !== undefined) {
          stat.n = stat.deletedCount;
        }
      }
  ...
  function prep_patterns (req, res, next) {
    ...
    function iter_regex (prefix, suffix) {
      ...
      function make (pat) {
        ...
        regexGuard.assertSafe(String(pat));
