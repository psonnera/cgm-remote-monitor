...
module.exports = function fitTreatmentsToBGCurve (ddata, env, ctx) {
  ...
  function updateTreatmentBG(treatment) {
    ...
    function mgdlByTime() {
      ...
      var treatmentMills = treatment.mills;
      if (!treatmentMills && treatment.created_at) treatmentMills = new Date(treatment.created_at).getTime();
      if (!treatmentMills && treatment.date) {
        treatmentMills = typeof treatment.date === 'number' ? treatment.date : new Date(treatment.date).getTime();
      }
      if (!treatmentMills || isNaN(treatmentMills)) return undefined;
      ...
      // Leave the result undefined when no usable BG could be derived; the
      // client falls back to the nearest SGV instead of a fabricated value.
      return Math.round(calcedBG) || undefined;
    }
