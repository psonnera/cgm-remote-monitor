...
const async = require('../async-compat');
...
function mergeProcessSort(oldData, newData, ageLimit) {
  ...
  if (oldData && filtered) {
    ...
    // Index new _ids for O(1) membership tests instead of a nested scan.
    const filteredIds = new Set();
    for (let j = 0; j < filtered.length; j++) {
        filteredIds.add('' + filtered[j]._id);
    }
    for (let i = 0; i < oldData.length; i++) {
        const oldElement = oldData[i];
        if (!filteredIds.has('' + oldElement._id)) {
...
function init(env, ctx) {
    ...
    dataloader.update = function update(ddata, opts, done) {
        ...
        function loadComplete(err, result) {
            ...
            // (dedupe by _id already happens in the cache merge via
            // ddata.idMergePreferNew; an older _.uniq(..., fn) call here used the
            // lodash-3 iteratee signature and was a no-op under lodash 4)
...
function loadDeviceStatus(ddata, env, ctx, callback) {
    ...
    ctx.devicestatus.list(opts, function(err, results) {
        if (!err && results) {
            ...
            // Derive/refresh the lean IOB/COB series for the records just loaded so
            // retro queries read tiny precomputed rows instead of full devicestatus.
            if (ctx.iobcob && r && r.length) {
                ctx.iobcob.upsertFromDeviceStatus(r, function (err) {
                    if (err) console.error('iobcob upsert failed', err && err.message ? err.message : err);
                });
            }
...
function loadDatabaseStats(ddata, ctx, callback) {
    // Mongo driver v6 dropped callbacks; db.stats() returns a promise.
    ctx.store.db.stats().then(function mongoDone (result) {
        if (result) {
            ddata.dbstats = {
