...
daytoday.css =
  '#daytodaycharts .daytoday-glucosebar {' +
  '  width: 0.55in;' +
  '  height: 0.9in;' +
  ...
  '  margin: 0 auto;' +
  '}' +
  '#daytodaycharts .daytoday-segment {' +
  '  width: 100%;' +
  '  display: flex;' +
  '  align-items: center;' +
  ...
  '  color: #333;' +
  '}' +
  '#daytodaycharts .daytoday-segment span {' +
  '  display: block;' +
  '  padding: 0 2px;' +
  '}' +
  '#daytodaycharts .daytoday-rangetable {' +
  '  margin: 5px auto;' +
  '  border-collapse: collapse;' +
  '  font-size: 16px;' +
  '}' +
  '#daytodaycharts .daytoday-rangetable td {' +
  '  padding: 2px 8px;' +
  '  text-align: right;' +
  '}' +
  '#daytodaycharts .daytoday-rangetable td:first-child {' +
  '  text-align: left;' +
  '  font-weight: bold;' +
  '}' +
  '#daytodaycharts .daytoday-statstable {' +
  '  margin: 10px auto;' +
  '  border-collapse: collapse;' +
  '  font-size: 16px;' +
  '}' +
  '#daytodaycharts .daytoday-statstable td {' +
  '  padding: 2px 8px;' +
  '  text-align: right;' +
  '}' +
  '#daytodaycharts .daytoday-statstable td:first-child {' +
  '  text-align: left;' +
  '  font-weight: bold;' +
  '}' +
  '#daytodaycharts table td {' +
  '  vertical-align: top;' +
  '  padding-left: 20px;' +
  '}' +
  '#daytodaycharts table td:first-child {' +
  '  padding-left: 0;' +
  '}' +
  '#daytodayinsulinstat table {' +
  '  font-size: 11px;' +
  '}' +
...
daytoday.html = function html (client) {
  ...
  var ret =
    ...
    '<br>' +
    '<details>' +
    '<summary style="cursor: pointer; font-weight: bold; font-size: 12pt; padding: 10px; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 5px;">' +
    translate('Display Options') +
    '</summary>' +
    '<div style="padding: 15px; border: 1px solid #ccc; border-top: none; background-color: #fafafa;">' +
    translate('Display') + ': ' +
...
daytoday.prepareHtml = function daytodayPrepareHtml (sorteddaystoshow) {
  ...
  sorteddaystoshow.forEach(function eachDay (d) {
    $('#daytodaycharts').append($('<table><tr><td><div id="daytodaychart-' + d + '"></div></td><td><div id="daytodayglucosestats-' + d + '"></div></td><td><div id="daytodayinsulinstat-' + d + '"></div></td></tr></table>'));
  });
};

daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  if (_renderDayHandle !== null) {
    clearTimeout(_renderDayHandle);
    _renderDayHandle = null;
  }
  ...
  // Modernized: Async chunked chart rendering for better UI responsiveness
  var dayIdx = 0;
  var chunkSize = 2; // Render 2 days at a time

  function renderDayChunk() {
    var end = Math.min(dayIdx + chunkSize, sorteddaystoshow.length);
    for (; dayIdx < end; dayIdx++) {
      var day = sorteddaystoshow[dayIdx];
      drawChart(day, datastorage[day], options);
    }

    if (dayIdx < sorteddaystoshow.length) {
      _renderDayHandle = setTimeout(renderDayChunk, 0);
    } else {
      _renderDayHandle = null;
      renderSummary();
    }
  ...
  function drawChart (day, data, options) {
    ...
    // add defs for combo boluses (only once — pattern is identical for every chart)
    if (d3.select('#hash').empty()) {
      var dashWidth = 5;
      d3.select('body').append('svg')
        .attr('id', 'nightscout-combo-defs')
    ...
    for (var dt = moment(from); dt < to; dt.add(5, 'minutes')) {
      ...
      // Calculate basal insulin and/or prepare graph data
      if (options.basal || options.insulindistribution) {
        var date = dt.format('x');
        var basalvalue = profile.getTempBasal(date);
        
        // Calculate total basal insulin delivered (5 minute increment)
        baseBasalInsulin += basalvalue.totalbasal * 5 / 60;

        // Prepare basal graph data (only if basal display is enabled)
        if (options.basal) {
    ...
    data.treatments.forEach(function(treatment) {
      // Calculate bolus stats - sum all insulin from treatments
      // (Temp Basal, Profile Switch, Notes, etc. don't have insulin field)
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
        ...
        if (treatment.duration && !treatment.cuttedby) {
          var endMills = times.mins(treatment.duration).msecs + treatment.mills;
          var endState = client.profilefunctions.profileSwitchStateAtTime
            ? client.profilefunctions.profileSwitchStateAtTime(endMills + 1)
            : { profile: client.profilefunctions.activeProfileToTime(endMills + 1) };
          var currentStateKey = client.profilefunctions.profileSwitchStateKey
            ? client.profilefunctions.profileSwitchStateKey(treatment)
            : client.profilefunctions.profileSwitchName(treatment.profile);
          var endStateKey = client.profilefunctions.profileSwitchStateKey
            ? client.profilefunctions.profileSwitchStateKey(endState)
            : client.profilefunctions.profileSwitchName(endState.profile);

          if (endState.profile && endStateKey !== currentStateKey) {
            appendProfileSwitch(context, Object.assign({}, endState, {
    ...
    // Calculate and render glucose distribution stats
    var glucoseStats = {
      lows: 0,
      normal: 0,
      highs: 0
    ...
    var bgValues = [];
    // Use statsrecords (filtered actual CGM data) instead of data.sgv (which may include predicted values)
    var statsRecords = data.statsrecords || [];
    statsRecords.forEach(function(record) {
    ...
    if (bgValues.length > 0) {
      ...
      glucoseStatsHtml += '<div class="daytoday-glucosebar">';
      glucoseStatsHtml += '<div class="daytoday-segment" style="height:' + lowPct + '%; background-color:#f88;">' + lowLabel + '</div>';
      glucoseStatsHtml += '<div class="daytoday-segment" style="height:' + normalPct + '%; background-color:#8f8;">' + normalLabel + '</div>';
      glucoseStatsHtml += '<div class="daytoday-segment" style="height:' + highPct + '%; background-color:#ff8; color:#000;">' + highLabel + '</div>';
      glucoseStatsHtml += '</div>';
      
      // Statistics table at bottom
      glucoseStatsHtml += '<table class="daytoday-statstable"><tbody>';
      ...
      $('#daytodayglucosestats-' + day).append(glucoseStatsHtml);
    }

    if (options.insulindistribution) {
      console.log('Insulin for day: ' + day + ' bolus: ' + bolusInsulin + ' basal: ' + baseBasalInsulin);
      var table = $('<table style="text-align: left;">');
      $('<tr><td colspan="2"><b>' + translate('Treatments') + ':</b></td></tr>').appendTo(table);
      $('<tr><td colspan="2">&nbsp;</td></tr>').appendTo(table);
      $('<tr><td colspan="2"><b>' + translate('Insulin') + ':</b></td></tr>').appendTo(table);
      $('<tr><td><b>' + translate('Total bolus') + ':</b></td><td align="right">' + bolusInsulin.toFixed(1) + 'U</td></tr>').appendTo(table);
      $('<tr><td><b>' + translate('Total basal') + ':</b></td><td align="right">' + baseBasalInsulin.toFixed(1) + 'U</td></tr>').appendTo(table);
      var totalDailyInsulin = bolusInsulin + baseBasalInsulin;
      $('<tr><td><b>' + translate('Total daily') + ':</b></td><td align="right">' + totalDailyInsulin.toFixed(1) + 'U</td></tr>').appendTo(table);
      $('<tr><td colspan="2">&nbsp;</td></tr>').appendTo(table);
      $('<tr><td><b>' + translate('Total carbs') + ':</b></td><td align="right">' + data.dailyCarbs + ' g</td></tr>').appendTo(table);
      
      $('#daytodayinsulinstat-' + day).append(table);
    }

    tddSum += totalDailyInsulin;
    basalSum += baseBasalInsulin;
    ...
    function appendProfileSwitch (context, treatment) {
      ...
      var displayProfile = client.profilefunctions.profileSwitchDisplay
        ? client.profilefunctions.profileSwitchDisplay(treatment)
        : client.profilefunctions.profileSwitchName(treatment.profile);
      var displayCutting = treatment.cutting
        ? (client.profilefunctions.profileSwitchDisplay
          ? client.profilefunctions.profileSwitchDisplay({ profile: treatment.cutting })
          : client.profilefunctions.profileSwitchName(treatment.cutting))
        : '';
      var text;
      if (treatment.cutting) {
        text = sign + '    ' + displayCutting + '    ' + '►►►' + '    ' + displayProfile + '    ' + sign;
      } else {
        text = sign + '    ' + displayProfile + '    ' + sign;
      }
      context.append('text')
        .style('font-size', 12)
        .style('font-weight', 'bold')
