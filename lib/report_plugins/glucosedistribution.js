...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var thead = $('<tr/>');
  $('<th>' + translate('Glucose (CGM)') + '</th>').appendTo(thead);
  ...
  for (var rangeIdx = 0; rangeIdx < ranges.length; rangeIdx++) {
    (function() {
      ...
      var range = rangeInfo.name;
      result[range] = {};
      var r = result[range];
      
      var minThreshold = rangeInfo.min;
      var maxThreshold = rangeInfo.max;
      
      r.rangeRecords = [];
      for (var i = 0; i < glucose_data.length; i++) {
        var rec = glucose_data[i];
        if (rec.bgValue >= minThreshold && rec.bgValue < maxThreshold) {
      ...
      r.localBgs = [];
      for (var j = 0; j < r.rangeRecords.length; j++) {
        if (r.rangeRecords[j].sgv) {
          r.localBgs.push(r.rangeRecords[j].sgv);
        }
      }
  ...
  for (var defIdx = 0; defIdx < rangeDefinitions.length; defIdx++) {
    ...
    var rangeExp = ' (' + rangeInfo.range + ' mg/dL)';

    $('<td class="tdborder"><strong>' + translate(rangeInfo.label) + rangeExp + ': </strong></td>').appendTo(tr);
    ...
    $('<td class="tdborder" style="' + bgColor + '">' + actualPct + '%</td>').appendTo(tr);
    
    // Target column 1
    $('<td class="tdborder">' + (rangeInfo.target1 || '') + '</td>').appendTo(tr);
    
    // Target column 2 with rowspan for combined targets
    if (!rangeInfo.target2Skip) {
      if (rangeInfo.target2Rowspan) {
        $('<td class="tdborder" rowspan="' + rangeInfo.target2Rowspan + '">' + (rangeInfo.target2 || '') + '</td>').appendTo(tr);
      } else {
        $('<td class="tdborder">' + (rangeInfo.target2 || '') + '</td>').appendTo(tr);
      }
    }
  ...
  $('<tr><th colspan="2">' + translate('Statistics') + '</th></tr>').appendTo(statsTable);
  
  if (glucose_data.length > 0) {
    $('<tr><td class="tdborder"><strong>' + translate('Average') + '</strong></td><td class="tdborder">' + avgValue + '</td></tr>').appendTo(statsTable);
    $('<tr><td class="tdborder"><strong>' + translate('Median') + '</strong></td><td class="tdborder">' + medianValue + '</td></tr>').appendTo(statsTable);
    $('<tr><td class="tdborder"><strong>' + translate('STDev.') + '</strong></td><td class="tdborder" style="' + stdBgColor + '">' + stdValue.toFixed(1) + '</td></tr>').appendTo(statsTable);
    $('<tr><td class="tdborder"><strong>' + translate('GMI') + '</strong></td><td class="tdborder" style="' + gmiBgColor + '">' + gmiDisplay + '</td></tr>').appendTo(statsTable);
    ...
    $('<tr><td class="tdborder"><strong>' + translate('GVI') + '</strong></td><td class="tdborder" style="' + gviBgColor + '">' + GVI + '</td></tr>').appendTo(statsTable);
    $('<tr><td class="tdborder"><strong>' + translate('PGS') + '</strong></td><td class="tdborder" style="' + pgsBgColor + '">' + PGS + '</td></tr>').appendTo(statsTable);
  }
  ...
  renderPercentileChart(datastorage, sorteddaystoshow, options);

  function renderPercentileChart(datastorage, sorteddaystoshow, options) {
    var minutewindow = 30;
    var data = datastorage.allstatsrecords;
    var bins = [];
    ...
    for (var hour = 0; hour < 24; hour++) {
      for (var minute = 0; minute < 60; minute = minute + minutewindow) {
        ...
        readings = readings.map(function(record) { return record.sgv; });
        bins.push([date, readings]);
      }
    }

    var dat10 = bins.map(function(bin) { return [bin[0], ss.quantile(bin[1], 0.1)]; });
    var dat25 = bins.map(function(bin) { return [bin[0], ss.quantile(bin[1], 0.25)]; });
    var dat50 = bins.map(function(bin) { return [bin[0], ss.quantile(bin[1], 0.5)]; });
    var dat75 = bins.map(function(bin) { return [bin[0], ss.quantile(bin[1], 0.75)]; });
    var dat90 = bins.map(function(bin) { return [bin[0], ss.quantile(bin[1], 0.9)]; });
    var datMin = bins.map(function(bin) { return [bin[0], bin[1].length > 0 ? ss.min(bin[1]) : null]; });
    var datMax = bins.map(function(bin) { return [bin[0], bin[1].length > 0 ? ss.max(bin[1]) : null]; });
    ...
    $.plot('#glucosedistribution-percentile-chart', [{
      ...
      label: 'Low (70)', data: [], color: '#214102'
    }, {
      label: 'High (180)', data: [], color: '#653403'
    }], {
      xaxis: { mode: 'time', timezone: 'browser', timeformat: '%H:%M', tickColor: '#555' },
      yaxis: { min: 40, max: options.units === 'mmol' ? Math.ceil(yMax / 18) : yMax, tickColor: '#555' },
      legend: { position: 'nw', margin: [10, 10], noColumns: 5 },
      grid: {
        markings: [{
          color: 'rgba(0, 100, 0, 0.08)', yaxis: { from: 70, to: 180 }
        }, {
          color: '#214102', lineWidth: 2, dashStyle: [5, 5], yaxis: { from: 70, to: 70 }
        }, {
          color: '#0a6b01', lineWidth: 2, dashStyle: [5, 5], yaxis: { from: 140, to: 140 }
        }, {
          color: '#653403', lineWidth: 2, dashStyle: [5, 5], yaxis: { from: 180, to: 180 }
        }]
      }
    });
  }
