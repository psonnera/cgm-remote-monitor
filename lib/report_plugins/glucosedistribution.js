...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<td><div id="glucosedistribution-daterange">(' +
    '<span id="glucosedistribution-days"></span>' +
    ')</div><div id="glucosedistribution-report"></div></td>' +
    '</tr>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  if (_validateHandle !== null) {
    clearTimeout(_validateHandle);
    _validateHandle = null;
  }
  if (_gviHandle !== null) {
    clearTimeout(_gviHandle);
    _gviHandle = null;
  }
  ...
  // Read the current popup value so we don't roll back when reportclient already advanced the bar.
  // Internal 0-100% progress is mapped into the remaining range (progressStart → 100%).
  var progressStart = parseInt($('#progress-text').text()) || 0;
  var lastProgressPercent = progressStart;

  // Helper function to update progress bar (as popup overlay)
  function updateProgress(percent, message) {
    // Map internal 0-100% to progressStart-100% so we never roll back from reportclient's value
    percent = progressStart + Math.round(percent * (100 - progressStart) / 100);
    // Never go backward
    if (percent < lastProgressPercent) {
      percent = lastProgressPercent;
    }
    lastProgressPercent = percent;
    // Check if progress popup exists, if not create it (skip if caller suppressed it)
  ...
  // Update or create the progress popup — never destroy reportclient's in-progress popup
  updateProgress(0, translate('Loading data') + '...');
  
  // Use async processing to avoid blocking UI
  setTimeout(function() {
    processGlucoseDistribution(datastorage, sorteddaystoshow, options, ss, translate, displayUnits, updateProgress, updateStatus);
  }, 0);
};

function processGlucoseDistribution(datastorage, sorteddaystoshow, options, ss, translate, displayUnits, updateProgress, updateStatus) {
  ...
  var days = sorteddaystoshow.length;
  ...
  loadValidatedData(continueAfterLoad);

  function loadValidatedData(done) {
    ...
    function validateChunk() {
      ...
      if (rawData.length > 0) {
        // Validation occupies internal 0-60%; GVI and rendering use 60-100%
        updateProgress(Math.round((idx / rawData.length) * 60), translate('Loading data') + '...');
      }

      if (idx < rawData.length) {
        _validateHandle = setTimeout(validateChunk, 0);
        return;
      }
      _validateHandle = null;
      ...
      updateProgress(60, translate('Loading data') + '...');
      done(validatedData);
    }
  ...
  function continueAfterLoad(validatedData) {
    if (validatedData.length === 0) {
      $('#glucosedistribution-days').text(translate('Result is empty'));
      updateStatus(translate('Complete'));
      setTimeout(function() {
    ...
    updateProgress(60, translate('Preparing display') + '...');
  ...
  // Create container div for side-by-side tables (create fresh for each render)
  ...
  if (glucose_data.length > 0) {
    ...
    stdValue = Math.round(ss.standardDeviation(localBgs) * 10) / 10;
    stdBgColor = '';
    if (stdValue <= 36) {
  ...
  // Wrap processing in a timeout for async behavior
  setTimeout(function() {
  // Modernized: Chunk GVI/PGS calculation for large datasets
  var events = 0;
  var GVITotal = 0;
  var GVIIdeal = 0;
  var GVIIdeal_Time = 0;
  ...
  var deltaTotal = 0;
  var gviIdx = 0;
  var gviChunkSize = 500;
  
  function processGVIChunk() {
    var end = Math.min(gviIdx + gviChunkSize, glucose_data.length - 1);
    for (; gviIdx < end; gviIdx++) {
      const entry = glucose_data[gviIdx];
      const nextEntry = glucose_data[gviIdx + 1];
    ...
    // GVI occupies internal 60-85%; advance proportionally through chunks
    var gviProgress = glucose_data.length > 1 ? gviIdx / (glucose_data.length - 1) : 1;
    updateProgress(60 + Math.round(gviProgress * 25), translate('Preparing display') + '...');

    if (gviIdx < glucose_data.length - 1) {
      _gviHandle = setTimeout(processGVIChunk, 0);
    } else {
      _gviHandle = null;
      finalizeGVICalculation();
    }
  }

  function finalizeGVICalculation() {
    ...
    renderStatsTables(GVI, PGS);
  }
  
  function renderStatsTables(GVI, PGS) {
    ...
    // Clear report right before appending to prevent race conditions on first load
    report.empty();
    report.append(tablesContainer);
    
    // Rendering occupies internal 85-100%: tables built, now render charts
    updateProgress(87, translate('Preparing display') + '...');

    // Create vertical stacked bar chart with 5 ranges using bright colors
    // Defer to next frame to avoid blocking
    requestAnimationFrame(function() {
    updateProgress(90, translate('Preparing display') + '...');
    try {
      var barRanges = [
      ...
      for (var idx = 0; idx < barRanges.length; idx++) {
        ...
        if (result[rangeName] && result[rangeName].readingspct) {
          ...
          if (pct > 0) {
            var labelMinPct = 10;
            var textColor = rangeName === 'High' ? '#000' : '#fff';
            var textShadow = rangeName === 'High' ? 'none' : '1px 1px 1px rgba(0,0,0,0.5)';
            barHTML += '<div style="height: ' + pct + '%; width: 100%; background-color: ' + graphcolors[rangeName] + '; display: flex; align-items: center; justify-content: center; color: ' + textColor + '; font-weight: bold; font-size: 12px; text-shadow: ' + textShadow + ';">';
            if (pct >= labelMinPct) barHTML += pct + '%';
            barHTML += '</div>';
          }
        }
      }
    ...
    // Render percentile chart with validated data
    renderPercentileChart(validatedData, options);
  }
  
  // Start chunked GVI processing
  processGVIChunk();
  }, 0);
  ...
  function renderPercentileChart(validatedData, options) {
    ...
    updateProgress(95, translate('Preparing display') + '...');
    });

    // safePlot schedules the actual plot call; hook completion via a short poll isn't
    // reliable across browsers — just advance to 100% and remove the popup here.
    requestAnimationFrame(function() {
