...
function processGlucoseDistribution(datastorage, sorteddaystoshow, options, ss, translate, displayUnits, updateProgress) {
  ...
  // Calculate adaptive sampling based on data size for optimal performance
  var dataSize = datastorage.allstatsrecords.length;
  var adaptiveSampleInterval = 1;
  if (dataSize > 20000) {
    adaptiveSampleInterval = 5; // 3 months: sample every 5th point (80% reduction)
  } else if (dataSize > 10000) {
    adaptiveSampleInterval = 3; // 1-2 months: sample every 3rd point (67% reduction)
  }
  // else: < 10k readings, process all data
  ...
  for (var defIdx = 0; defIdx < rangeDefinitions.length; defIdx++) {
    ...
    tableHTML += '<tr>';
    tableHTML += '<td class="tdborder"><strong>' + translate(rangeInfo.label) + rangeExp + ': </strong></td>';
  ...
  if (glucose_data.length > 0) {
    // Use adaptive sampling for statistics
    var localBgs = [];
    var mgDlBgs = [];
    for (var i = 0; i < glucose_data.length; i += adaptiveSampleInterval) {
  ...
  for (i = 0; i <= glucose_data.length - 2; i += adaptiveSampleInterval) {
    const entry = glucose_data[i];
    const nextEntry = glucose_data[Math.min(i + adaptiveSampleInterval, glucose_data.length - 1)];
    const timeDelta = nextEntry.displayTime.getTime() - entry.displayTime.getTime();

    if (timeDelta == 0 || timeDelta > maxGap * adaptiveSampleInterval) {
      continue;
    }
  ...
  function renderPercentileChart(validatedData, options) {
    ...
    // Use adaptive sampling for percentile chart
    var timeSlots = {};
    for (var i = 0; i < data.length; i += adaptiveSampleInterval) {
    ...
    requestAnimationFrame(function() {
      safePlot('#glucosedistribution-percentile-chart', [{
