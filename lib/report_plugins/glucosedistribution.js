...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var lastProgressPercent = 0; // Track progress to prevent going backward
  
  // Helper function to update progress bar (as popup overlay)
  function updateProgress(percent, message) {
    // Round to whole number to avoid decimal display
    percent = Math.round(percent);
    // Detect restart: if we had high progress and now getting low percent, reset
    if (percent < 10 && lastProgressPercent > 80) {
      lastProgressPercent = 0;
    }
    // Never go backward (unless restarting)
  ...
  // Clean up any existing progress popup before starting
  $('#progress-popup-overlay').remove();
  
  // Show initial progress - Phase 1: Loading data
...
function processGlucoseDistribution(datastorage, sorteddaystoshow, options, ss, translate, displayUnits, updateProgress, updateStatus) {
  ...
  function loadValidatedData(done) {
    ...
    function validateChunk() {
      ...
      if (rawData.length > 0) {
        updateProgress((idx / rawData.length) * 100, translate('Loading data') + '...');
      }
      ...
      updateProgress(100, translate('Loading data') + '...');
      done(validatedData);
    }
  ...
  function continueAfterLoad(validatedData) {
    ...
    updateStatus(translate('Preparing display') + '...');
  ...
  function renderPercentileChart(validatedData, options) {
    ...
    // Complete and clear progress popup immediately
    updateStatus(translate('Complete'));
    setTimeout(function() {
      $('#progress-popup-overlay').remove();
    }, 0);
