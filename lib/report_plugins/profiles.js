...
profiles.report = function report_profiles (datastorage) {
  _profileGraphHandles.forEach(function(h) { clearTimeout(h); });
  _profileGraphHandles = [];
  ...
  // Find the most recent profile (latest startDate)
  var mostRecentIndex = 0;
  var mostRecentDate = new Date(profileRecords[0].startDate);
  
  for (var r = 0; r < profileRecords.length; r++) {
    var startDate = new Date(profileRecords[r].startDate);
    databaseRecords.append('<option value="' + r + '">' + translate('Valid from:') + ' ' + startDate.toLocaleString() + '</option>');
  ...
  // Select the most recent profile
  databaseRecords.val(mostRecentIndex);
  ...
  recordChange(null, activeProfileName);

  function recordChange (event, initialActiveProfile) {
    ...
    // Set active profile as selected if this is initial load
    if (initialActiveProfile && profileNames.includes(initialActiveProfile)) {
      profileSelector.val(initialActiveProfile);
    } else if (currentrecord.defaultProfile && profileNames.includes(currentrecord.defaultProfile)) {
      // Otherwise fall back to default profile
  ...
  function createDataSection(title, data, type) {
    ...
    _profileGraphHandles.push(setTimeout(function() {
  ...
  function renderGraph(graphId, data) {
    try {
      ...
      var container = $('#' + graphId);
      if (container.length === 0 || container.width() === 0 || container.height() === 0) {
        console.warn('Profile graph container not ready:', graphId);
        return;
      }
  ...
  function renderTargetGraph(graphId, targetLow, targetHigh) {
    try {
      ...
      var container = $('#' + graphId);
      if (container.length === 0 || container.width() === 0 || container.height() === 0) {
        console.warn('Profile target graph container not ready:', graphId);
        return;
      }
