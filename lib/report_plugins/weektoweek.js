...
function init (ctx) {
  ...
  weektoweek.html = function html (client) {
    ...
    var ret =
      ...
      '<summary style="cursor: pointer; font-weight: bold; font-size: 12pt; margin-bottom: 10px;">' +
      ...
      '<br>' +
      translate('Scale') + ': ' +
      '<label><input type="radio" name="wrp_scale" id="wrp_linear" checked>' +
      translate('Linear') + '</label>' +
      '<label><input type="radio" name="wrp_scale" id="wrp_log">' +
  ...
  weektoweek.prepareHtml = function weektoweekPrepareHtml (weekstoshow) {
    ...
    legend += '<td><svg width="16" height="16"><circle fill="' + dayColors[colorIdx++] + '" r="40"></circle></g></svg></td><td>' + translate('Wednesday') + '</td>';
    legend += '<td><svg width="16" height="16"><circle fill="' + dayColors[colorIdx++] + '" r="40"></circle></g></svg></td><td>' + translate('Thursday') + '</td>';
  ...
  weektoweek.report = function report_weektoweek (datastorage, sorteddaystoshow, options) {
    if (_renderWeekHandle !== null) {
      clearTimeout(_renderWeekHandle);
      _renderWeekHandle = null;
    }
    ...
    function renderWeekChunk() {
      ...
      if (weekIdx < weekstoshow.length) {
        _renderWeekHandle = setTimeout(renderWeekChunk, 0);
      } else {
        _renderWeekHandle = null;
      }
    }
