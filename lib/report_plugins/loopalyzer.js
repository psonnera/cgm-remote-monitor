...
loopalyzer.html = function html (client) {
  ...
  ret += '<br/><br/><b>';
  ret += translate('To update this report, press SHOW while in this view');
  ret += '</b></span>';
...
loopalyzer.prepareHtml = function loopalyzerPrepareHtml () {
  // Always recreate chart containers to ensure they exist
  var charts = $('#loopalyzer-charts');
  if (charts.length > 0) {
    charts.empty();
    charts.append('<div class="chart" id="loopalyzer-basal" style="height:100px;margin-bottom:-14px;"></div>');
    charts.append('<div class="chart" id="loopalyzer-bg" style="height:200px;margin-bottom:-14px;"></div>');
    charts.append('<div class="chart" id="loopalyzer-tempbasal" style="height:150px;margin-bottom:-14px;"></div>');
    charts.append('<div class="chart" id="loopalyzer-iob" style="height:150px;margin-bottom:-14px;"></div>');
    charts.append('<div class="chart" id="loopalyzer-cob" style="height:150px;"></div>');
  }
};
...
loopalyzer.renderProfilesTable = function(datastoreProfiles, daysToShow, client) {
  ...
  if (client.profilefunctions && client.profilefunctions.activeProfileToTime) {
    activeProfileName = client.profilefunctions.activeProfileToTime();
  }
  ...
  datastoreProfiles.forEach(function(entry) {
    ...
    if (store) {
      // First try to use the active profile, otherwise use defaultProfile, otherwise use first available
      var profileToUse = null;
      if (activeProfileName && store[activeProfileName]) {
        profileToUse = activeProfileName;
      } else if (entry.defaultProfile && store[entry.defaultProfile]) {
        profileToUse = entry.defaultProfile;
      } else {
      ...
      if (profileToUse && store[profileToUse]) {
        var defaultProfile = store[profileToUse];
        newEntry.profileName = profileToUse;
  ...
  if (days <= 14) {
    ...
    function updateProgress(percent, message) {
      ...
      if ($('#progress-bar').length === 0) {
        $('#info').html('<b>' + message + '</b><br><div id="progress-container" style="width: 100%; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 5px; margin-top: 10px; height: 30px;"><div id="progress-bar" style="width: ' + percent + '%; height: 100%; background-color: #6c6; border-radius: 5px; transition: width 0.3s ease; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;"><span id="progress-text">' + percent + '%</span></div></div>');
      } else {
    ...
    var translate = client.translate;
    updateProgress(0, translate('Rendering Loopalyzer') + '...');
    
    // Create containers first, then generate report
    loopalyzer.prepareHtml();
    
    // Wait for containers to have dimensions before generating report
    setTimeout(function() {
      loopalyzer.generateReport(datastorage, daysToShow, options, updateProgress);
    }, 50);
  } else {
...
loopalyzer.generateReport = function(datastorage, daysToShow, options, updateProgress) {
  var Nightscout = window.Nightscout;
  var client = Nightscout.client;
  var translate = client.translate;
  ...
  if (updateProgress) updateProgress(10, translate('Rendering Loopalyzer') + '...');
  ...
  if (updateProgress) updateProgress(20, translate('Rendering Loopalyzer') + '...');

  // Split heavy calculations into async chunks for progress visibility
  setTimeout(function() {
    ...
    if (updateProgress) updateProgress(30, translate('Rendering Loopalyzer') + '...');
    
    setTimeout(function() {
      var sgvBin = loopalyzer.getSGVs(datastorage, daysToShow);
      var basalsBin = loopalyzer.getBasals(datastorage, daysToShow, profile);
      if (updateProgress) updateProgress(40, translate('Rendering Loopalyzer') + '...');
      ...
      setTimeout(function() {
        ...
        if (updateProgress) updateProgress(50, translate('Rendering Loopalyzer') + '...');
        ...
        setTimeout(function() {
          ...
          if (updateProgress) updateProgress(60, translate('Rendering Loopalyzer') + '...');
          
          setTimeout(function() {
            continueWithTimeShiftAndRendering(carbTreatments, insulinTreatments, sgvBin, basalsBin, tempBasalsBin, iobBin, cobBin, predictionsBin);
          }, 0);
        }, 0);
      }, 0);
    }, 0);
  ...
  function continueWithTimeShiftAndRendering(carbTreatments, insulinTreatments, sgvBin, basalsBin, tempBasalsBin, iobBin, cobBin, predictionsBin) {
  ...
  if (updateProgress) updateProgress(70, translate('Rendering Loopalyzer') + '...');
  ...
  setTimeout(function() {
    ...
    if (updateProgress) updateProgress(80, translate('Rendering Loopalyzer') + '...');
  ...
  function safePlot(selector, data, options, retryCount) {
    ...
    if (container.length === 0) {
      if (retryCount < 3) {
        setTimeout(function() {
          safePlot(selector, data, options, retryCount + 1);
        }, 50);
      } else {
        console.error('Loopalyzer: Container not found after retries:', selector, '- DOM state:', $('#loopalyzer-charts').length > 0 ? 'charts div exists' : 'charts div missing');
      }
      return;
    }
    ...
    if (width === 0 || height === 0) {
      if (retryCount < 3) {
        ...
        console.error('Loopalyzer: Container has no dimensions after retries:', selector, 'width:', width, 'height:', height);
      }
      return;
    }
    ...
    if (data && Array.isArray(data)) {
      for (var i = 0; i < data.length; i++) {
        if (!data[i].color) {
          data[i].color = '#000000'; // Default to black if color missing
        }
      }
    }
    ...
    try {
      ...
      console.error('Loopalyzer: Error plotting chart:', selector, e);
    }
  }
  ...
  // Defer plotting to ensure DOM is ready
  requestAnimationFrame(function() {
    safePlot('#loopalyzer-basal', chartBasalData, chartBasalOptions);
  });
  ...
  requestAnimationFrame(function() {
    safePlot('#loopalyzer-bg', chartBGData, chartBGOptions);
  });
  ...
  requestAnimationFrame(function() {
    safePlot('#loopalyzer-tempbasal', chartTempBasalData, chartTempBasalOptions);
  });
  ...
  requestAnimationFrame(function() {
    safePlot('#loopalyzer-iob', chartIOBData, chartIOBOptions);
  });
  ...
  requestAnimationFrame(function() {
    safePlot('#loopalyzer-cob', chartCOBData, chartCOBOptions);
    
    // Complete and clear progress
    if (updateProgress) {
      setTimeout(function() {
        updateProgress(100, translate('Rendering Loopalyzer') + '...');
        setTimeout(function() {
          $('#info').html('');
        }, 500);
      }, 100);
  ...
  }, 0); // end setTimeout for averaging
  } // end continueWithTimeShiftAndRendering
};
