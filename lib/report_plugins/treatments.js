...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  function editTreatment(event) {
    ...
    var $dlg = $('#rp_edittreatmentdialog');
      ...
      ,  buttons: [
        { text: translate('Save'),
          ...
          click: function() {
            ...
            data.glucoseType = $dlg.find('input[name=rp_bginput]:checked').val();
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    if (options.enableeventtype && options.eventtype) {
      var eventtypeRegex = new RegExp(options.eventtype.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
      treatments = treatments.filter(function(tr) {
        return tr.eventType && eventtypeRegex.test(tr.eventType);
      });
    }
    ...
    table.append($('<tr>')
      .append($('<td>').attr('colspan','10').css('background','lightgray')
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      // Build notes with Bolus Wizard and Temporary Target details
      var notes = tr.notes ? tr.notes : '';
      
      // Add Temporary Target information
      if (tr.eventType === 'Temporary Target' && (tr.targetTop || tr.targetBottom)) {
        var targetText = '';
        if (tr.targetBottom && tr.targetTop) {
          if (tr.targetBottom === tr.targetTop) {
            targetText = 'Target: ' + tr.targetTop;
          } else {
            targetText = 'Target: ' + tr.targetBottom + ' - ' + tr.targetTop;
          }
        } else if (tr.targetTop) {
          targetText = 'Target: ' + tr.targetTop;
        } else if (tr.targetBottom) {
          targetText = 'Target: ' + tr.targetBottom;
        }
        
        if (tr.reason) {
          targetText += ' (' + translate(tr.reason) + ')';
        }
        
        if (targetText) {
          notes = notes ? notes + '<br><i>' + targetText + '</i>' : '<i>' + targetText + '</i>';
        }
      }
      
      var calc = null;
      
      // Check for Nightscout boluscalc format
      if (tr.boluscalc) {
        calc = tr.boluscalc;
      }
      // Check for AAPS bolusCalculatorResult format (JSON string)
      else if (tr.bolusCalculatorResult) {
        try {
          calc = JSON.parse(tr.bolusCalculatorResult);
        } catch (e) {
          console.error('Error parsing bolusCalculatorResult:', e);
        }
      }
      
      if (calc) {
        // Handle both Nightscout and AAPS field names
        var carbsAmount = calc.carbs || 0;
        var totalInsulin = calc.insulin || calc.totalInsulin;
        var percentageCorrection = calc.percentageCorrection;
        
        var wizardText = '';
        if (carbsAmount !== 0) {
          wizardText = 'Carbs: ' + carbsAmount.toFixed(0) + 'g';
        }
        
        if (totalInsulin !== undefined) {
          var insulinText = '-> ' + totalInsulin.toFixed(2) + 'U';
          if (percentageCorrection !== undefined && percentageCorrection !== 100) {
            insulinText += ' (' + percentageCorrection + '%)';
          }
          wizardText = wizardText ? wizardText + ' ' + insulinText : insulinText;
        }
        
        if (wizardText) {
          notes = notes ? notes + '<br><i>' + wizardText + '</i>' : '<i>' + wizardText + '</i>';
        }
      }
