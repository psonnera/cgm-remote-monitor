...
success.css =
  ...
  '  height: 0.9in;' +
  '}' +
  '#success-placeholder .success-bar {' +
  '  width: 0.55in;' +
  '  height: 0.9in;' +
  ...
  '}' +
  '#success-placeholder .success-segment {' +
  '  width: 100%;' +
  '  display: flex;' +
  '  align-items: center;' +
  ...
  '  color: #333;' +
  '}' +
  '#success-placeholder .success-segment span {' +
  '  display: block;' +
  '  padding: 0 2px;' +
  '}';
...
success.report = function report_success (datastorage, sorteddaystoshow, options) {
  ...
  var weekIdx = 0;
  var chunkSize = 3; // Process 3 weeks at a time
  
  function renderWeekChunk() {
    var end = Math.min(weekIdx + chunkSize, weekArray.length);
    for (; weekIdx < end; weekIdx++) {
      var weekInfo = weekArray[weekIdx];
      processWeekStats(weekInfo);
    }
    
    if (weekIdx < weekArray.length) {
  ...
  function processWeekStats(weekInfo) {
    var tr = $('<tr>');
    ...
    var barHtml = '<div class="success-bar">' +
      '<div class="success-segment" style="height:' + lowPct + '%; background-color:#f88;">' + lowLabel + '</div>' +
      '<div class="success-segment" style="height:' + normalPct + '%; background-color:#8f8;">' + normalLabel + '</div>' +
      '<div class="success-segment" style="height:' + highPct + '%; background-color:#ff8; color:#000;">' + highLabel + '</div>' +
      '</div>';
    $('#weeklystat-chart-' + weekStart).html(barHtml);
  }
