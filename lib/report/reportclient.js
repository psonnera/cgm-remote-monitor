...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    function switchreport_handler (event) {
      ...
      if (id === 'daytoday' || id === 'weektoweek' || id === 'dailystats') {
      ...
      if (id === 'glucosedistribution') {
        ...
        // Day to day: select today and set size to 800x250
        $('.presetdates').removeClass('selected');
        $('.presetdates[days="1"]').addClass('selected');
        setDataRange(null, 1);
        $('#rp_enabledate').prop('checked', true);
        $('#rp_size').val('800x250').find('option').each(function() {
          if ($(this).attr('x') === '800' && $(this).attr('y') === '250') {
