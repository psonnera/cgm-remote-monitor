...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    function prefetchRange (options, callback) {
      ...
      prefetchDays = {};
      var tz = client.sbx.data.profile.getTimezone();
      var needDevicestatus = !!(options.iob || options.cob || options.openAps || options.predicted);
      ...
      chunks.forEach(function (c) {
        pending.push($.ajax('/api/v1/entries.json?find[date][$gte]=' + c.from + '&find[date][$lt]=' + c.to + '&count=20000', {
          headers: client.headers()
          , success: function (xhr) {
            xhr.forEach(function (e) { if (e && e.date !== undefined) bucket('entries', e.date, e); });
          }
        }));
        pending.push($.ajax('/api/v1/treatments.json?find[created_at][$gte]=' + new Date(c.from).toISOString() + '&find[created_at][$lt]=' + new Date(c.to).toISOString() + '&count=5000', {
          headers: client.headers()
          , cache: false
          , success: function (xhr) {
        ...
        if (needDevicestatus) {
          pending.push($.ajax('/api/v1/devicestatus.json?find[created_at][$gte]=' + new Date(c.from).toISOString() + '&find[created_at][$lt]=' + new Date(c.to).toISOString() + '&count=20000', {
            headers: client.headers()
            , success: function (xhr) {
      ...
      $.when.apply($, pending).done(function () {
        if (window.reportPerf && perfStart) {
          ...
          console.log('[reportPerf] prefetch:', pending.length, 'requests,', days.length, 'days,', rows, 'rows in', Math.round(performance.now() - perfStart) + 'ms');
        }
        callback();
      }).fail(function () {
        prefetched = {};
