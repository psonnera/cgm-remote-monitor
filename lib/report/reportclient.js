  ...
  client.init(function loaded () {
    ...
    function prepareGUI () {
      $('.presetdates').click(function(event) {
        ...
        // Auto-trigger show() for most reports when preset date button is clicked
        // Exception: Loopalyzer requires manual SHOW click
        var currentReport = $('.menutab.selected').attr('id');
        if (currentReport !== 'loopalyzer') {
      ...
      $('#rp_to').on('change', updateNextButtonVisibility);
      ...
      // Set "Last month" button as selected by default
    ...
    function show (event) {
      ...
      function display () {
        var count = 0;
        sorteddaystoshow = [];
        $('#info').html('<b>' + translate('Loading data') + '...</b><br><div id="progress-container" style="width: 100%; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 5px; margin-top: 10px; height: 30px; position: relative;"><div id="progress-bar" style="width: 0%; height: 100%; background-color: #6c6; border-radius: 5px; transition: width 0.3s ease;"></div><span id="progress-text" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #333; font-weight: bold; z-index: 1;">0%</span></div>');
      ...
      function dataLoadedCallback (day) {
        ...
        // Update progress bar
        var progress = Math.round((loadeddays / dayscount) * 100);
        ...
        if (loadeddays === dayscount) {
          ...
          // Update progress to show profile loading phase
          $('#info').html('<b>' + translate('Loading profiles') + '...</b><br><div id="progress-container" style="width: 100%; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 5px; margin-top: 10px; height: 30px; position: relative;"><div id="progress-bar" style="width: 0%; height: 100%; background-color: #6c6; border-radius: 5px; transition: width 0.3s ease;"></div><span id="progress-text" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #333; font-weight: bold; z-index: 1;">0%</span></div>');
          ...
          function updateProfileProgress() {
            profilePhases++;
            var progress = Math.round((profilePhases / totalProfilePhases) * 100);
            $('#progress-bar').css('width', progress + '%');
            $('#progress-text').text(progress + '%');
          }
          ...
          loadProfileSwitch(dFrom, function loadProfileSwitchCallback () {
            ...
            loadProfilesRange(dFrom, dTo, sorteddaystoshow.length, function loadProfilesCallback () {
              $('#info').html('<b>' + translate('Preparing data display') + '...</b><br><div id="progress-container" style="width: 100%; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 5px; margin-top: 10px; height: 30px; position: relative;"><div id="progress-bar" style="width: 0%; height: 100%; background-color: #6c6; border-radius: 5px; transition: width 0.3s ease;"></div><span id="progress-text" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #333; font-weight: bold; z-index: 1;">0%</span></div>');
              window.setTimeout(function() {
                showreports(options);
              }, 0);
            }, updateProfileProgress);
    ...
    function navigatePrev (event) {
      ...
      $('#rp_from').val(from.subtract(duration + 1, 'days').format('YYYY-MM-DD'));
      $('#rp_to').val(to.subtract(duration + 1, 'days').format('YYYY-MM-DD'));
    ...
    function navigateNext (event) {
      ...
      $('#rp_from').val(from.add(duration + 1, 'days').format('YYYY-MM-DD'));
      $('#rp_to').val(to.add(duration + 1, 'days').format('YYYY-MM-DD'));
    ...
    function switchreport_handler (event) {
      ...
      if (id === 'loopalyzer') {
        // For loopalyzer, only show up to 2 weeks
        $('.presetdates[days="31"]').hide();
        $('.presetdates[days="90"]').hide();
      } else if (id === 'daytoday' || id === 'weektoweek' || id === 'dailystats') {
        // For day to day, week to week, and daily stats, hide 3 months button
        $('.presetdates[days="90"]').hide();
      ...
      if (id === 'glucosedistribution') {
        ...
        $('.presetdates').removeClass('selected');
        var firstOfThisMonth = moment().startOf('month');
        var lastOfPrevMonth = firstOfThisMonth.clone().subtract(1, 'day');
        var firstOfPrevMonth = lastOfPrevMonth.clone().startOf('month');
        $('#rp_from').val(firstOfPrevMonth.format('YYYY-MM-DD'));
        $('#rp_to').val(lastOfPrevMonth.format('YYYY-MM-DD'));
        $('#rp_enabledate').prop('checked', true);
      ...
      } else if (id === 'loopalyzer') {
        // Loopalyzer: select 2 weeks (maximum allowed), but do NOT auto-run
        $('.presetdates').removeClass('selected');
        $('.presetdates[days="14"]').addClass('selected');
        setDataRange(null, 14);
        $('#rp_enabledate').prop('checked', true);
        // Do NOT auto-run show() for loopalyzer - user must click SHOW manually
      } else if (id === 'treatments') {
        // Treatments: select 1 week
        $('.presetdates').removeClass('selected');
        $('.presetdates[days="7"]').addClass('selected');
        setDataRange(null, 7);
        $('#rp_enabledate').prop('checked', true);
        // Load data if not already available
        window.setTimeout(function() {
      ...
      } else if (id === 'weektoweek' || id === 'dailystats' || id === 'profiles' || 
                 id === 'success' || id === 'calibrations') {
        // Week to week, Daily Stats, Profiles, Weekly Distribution, Calibrations: select 1 month
        $('.presetdates').removeClass('selected');
        $('.presetdates[days="31"]').addClass('selected');
        setDataRange(null, 31);
        $('#rp_enabledate').prop('checked', true);
