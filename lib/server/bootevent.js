...
function boot (env, language) {
  ...
  function checkNodeVersion (ctx, next) {
    ...
    if (isBun) {
      console.debug('Running under Bun runtime');
      next();
      return;
    }
  ...
  function setupStorage (ctx, next) {
    ...
    try {
      if (_.startsWith(env.storageURI, 'openaps://')) {
        // openaps:// file-based storage (backed by the mongomock dependency)
        // has been removed. Fail with a clear message instead of crashing on a
        // missing module.
        const msg = 'openaps:// storage is no longer supported; use a mongodb:// storageURI';
        console.info(msg);
        ctx.bootErrors = ctx.bootErrors || [ ];
        ctx.bootErrors.push({ 'desc': 'Unsupported storage', err: msg });
        next();
      } else {
  ...
  function setupInternals (ctx, next) {
    ...
    ctx.iobcob = require('./iobcob')(env.iobcob_collection, ctx);
    ...
    ctx.memmon = require('./memmon')(env, ctx);
  ...
  function ensureIndexes (ctx, next) {
    ...
    ctx.store.ensureIndexes(ctx.iobcob( ), ctx.iobcob.indexedFields);
  ...
  function backfillIobCob (ctx, next) {

    console.log('Executing backfillIobCob');

    if (hasBootErrors(ctx)) {
      return next();
    }

    if (env.iobcob_autobackfill === false) {
      console.info('iobcob auto-backfill disabled (MONGO_IOBCOB_AUTO_BACKFILL=false)');
      return next();
    }

    var devicestatus = ctx.store.collection(env.devicestatus_collection);

    ctx.iobcob.needsBackfill(devicestatus, function (err, needed) {
      if (err) {
        // Non-fatal: a failed check must not block the server from booting.
        console.warn('iobcob backfill check failed (non-fatal):', err.message || err);
        return next();
      }
      if (!needed) {
        return next();
      ...
      console.info('iobcob collection empty but devicestatus has data: starting one-time backfill in background');

      // Run in the background so a large devicestatus history doesn't delay
      // startup. The backfill upserts idempotently and the runtime ingest path
      // keeps the collection current in the meantime, so it is safe to let it
      // finish after the server is already serving requests.
      ctx.iobcob.backfillFromDeviceStatus(devicestatus, {
        onProgress: function (p) {
          console.info('  iobcob backfill progress: scanned ' + p.scanned + ', upserted ' + p.upserted);
        }
      }, function (bErr, result) {
        if (bErr) {
          console.error('iobcob backfill failed (non-fatal):', bErr.message || bErr);
          if (ctx.adminnotifies) {
            ctx.adminnotifies.addNotify({
              title: 'IOB/COB backfill failed'
              , message: 'The one-time IOB/COB history backfill did not complete. Historical IOB/COB chart lines may be incomplete until it succeeds (restart to retry, or run bin/backfill-iobcob.js). Error: ' + (bErr.message || bErr)
              , persistent: true
            });
          }
          return;
        }
        console.info('iobcob backfill complete: scanned ' + result.scanned + ', upserted ' + result.upserted + ' rows');
      });

      // Don't wait for the backfill to finish before continuing boot.
      next();
    });
  }
  ...
  function setupListeners (ctx, next) {
    ...
    // Only one dataloader.update may run at a time: concurrent runs share
    // ctx.ddata (cleared mid-load), and every run allocates the full working
    // set. If writes arrive mid-load, run once more when the load finishes.
    var loading = false;
    ...
    var updateData = _.debounce(function debouncedUpdateData ( ) {
      ...
      // maxWait: a plain trailing debounce postpones on every call, so a
      // steady stream of uploads arriving < 5s apart would starve reloads
      // entirely; this guarantees at most-and-at-least one run per window.
    }, UPDATE_THROTTLE, { maxWait: UPDATE_THROTTLE });
    ...
    ctx.bus.on('data-received', function forceReloadData (evt) {
      console.info('got data-received event, requesting reload');
      updateData();
      // User-triggered writes (careportal/profile switch POSTs) propagate to
      // clients immediately; machine uploads (AAPS via API v3) coalesce under
      // the debounce so continuous uploading doesn't trigger a full reload
      // per request.
      if (evt && evt.immediate && updateData.flush) {
        updateData.flush();
      }
    });
