...
function init (env, ctx) {
  ...
  // One provider for the process lifetime: apn.Provider holds an HTTP/2
  // session with its own keep-alive timers, so creating one per notification
  // leaks a live connection each time. Options come from env, which is fixed
  // after boot.
