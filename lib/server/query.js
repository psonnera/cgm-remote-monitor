...
const regexGuard = require('./regex-guard');
...
function default_options (opts) {
  ...
  if (opts) {
    ...
    // Infer useEpoch from walker configuration if not explicitly set
    // If the dateField has parseInt as walker, assume epoch timestamps
    if (keys.indexOf('useEpoch') < 1 && opts.walker && opts.walker[opts.dateField] === parseInt) {
      opts.useEpoch = true;
    }
  }
  return opts;
...
function enforceDateFilter (query, opts) {
  ...
  // rewrite dates to ISO UTC strings or epoch numbers so queries work as expected
  if (dateValue) {
    Object.keys(dateValue).forEach(function(key) {
      ...
      if (typeof dateString === 'string') {
        // Check if it's a numeric string (epoch timestamp)
        const asNumber = Number(dateString);
        if (!isNaN(asNumber) && dateString === asNumber.toString()) {
          // It's a pure numeric string - convert to number if useEpoch, otherwise to ISO
          if (opts.useEpoch) {
            dateValue[key] = asNumber;
          } else {
            dateValue[key] = new Date(asNumber).toISOString();
          }
        } else {
          // It's an ISO/date string - parse and normalize
          let normalizedString = dateString.replace(' ', '+'); // some clients don't escape the plus

          const d = moment.parseZone(normalizedString);
          ...
          if (opts.useEpoch) {
            // Convert ISO string to epoch number
            dateValue[key] = d.valueOf();
          } else {
        ...
        // Already a number - convert to ISO if needed
        if (!opts.useEpoch) {
          dateValue[key] = new Date(dateString).toISOString();
        }
        // Otherwise leave as number for epoch-based queries
      }
    });
  }
...
function updateIdQuery (query) {
  if (query._id && query._id.length) {
    // only coerce valid 24-hex ids; leave anything else untouched so a
    // malformed id matches nothing instead of throwing a BSONError
    const objId = toObjectId(query._id);
    if (objId) {
      query._id = objId;
    }
  }
...
function create (params, opts) {
  ...
  // Get the initial query from params
  var query = params && params.find ? params.find : { };
  
  // Normalize date filters BEFORE type conversion to handle mixed date formats correctly
  // This prevents parseInt from corrupting ISO date strings (e.g., "2015-08-10T..." -> 2015)
  if (! query._id ) {
    enforceDateFilter(query, opts);
  }
  
  // Now apply walker type conversions (date values are already normalized)
  var finder = walker(opts.walker)({ find: query });
  // Get the final query to pass to mongodb.
  query = finder && finder.find ? finder.find : { };
      ...
      if (typeof opts.find[prop] === 'string') {
        ...
        traverse(opts.find[prop]).forEach(function (x) {
          ...
          if (this.isLeaf) {
            ...
            // This prevents re-converting values already normalized by enforceDateFilter
            if (typer === parseInt && typeof x === 'number') {
              // Already a number, skip conversion
              return;
            }
...
function parseRegEx (str) {
  ...
  if (regtest) {
    // reject pathological / overly long user patterns before compiling
    regexGuard.assertSafe(regtest[1]);
