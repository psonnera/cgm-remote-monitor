...
function storage (env, ctx) {
  ...
  function upsert (obj, fn) {
    ...
    if (isSensorEvent) {
      ...
      api().findOne(duplicateQuery, function(findErr, existingDoc) {
        if (findErr) {
      ...
      if (!err && obj.preBolus) {
        ...
        api( ).replaceOne(query, pbTreat, {upsert: true}, function pbComplete (err, updateResults) {
          ...
          try {
            ...
            if (ids.length > 0) {
              api().find({ _id: { $in: ids } }).toArray(function (findErr, docs) {
        ...
        try {
          if (obj && obj._id) {
            ...
            api().findOne({ _id: objId }, function (findErr, saved) {
              if (!findErr && saved) {
                ctx.bus.emit('data-update', {
                  type: 'treatments',
                  op: 'update',
  ...
  function save (obj, fn) {
    ...
    api().replaceOne({ _id: obj._id }, obj, { upsert: true }, saved);
