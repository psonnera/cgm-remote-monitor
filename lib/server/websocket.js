        ...
        if (opts.devicestatus && (opts.from || opts.to)) {
          try {
            ...
            if (opts.from && opts.to) {
              // Query both mills and created_at fields to handle different devicestatus formats
              query = {
                $or: [
                  { mills: { $gte: Number(opts.from), $lte: Number(opts.to) } },
              ...
              query = {
                $or: [
                  { mills: { $lte: Number(opts.to) } },
                  { created_at: { $lte: new Date(Number(opts.to)).toISOString() } }
                ]
              };
            }

            ctx.store.collection(env.devicestatus_collection).find(query).sort({ mills: 1, created_at: 1 }).toArray(function(err, docs) {
              if (err) {
                console.error('loadRetro devicestatus query error', err);
                socket.compress(true).emit('retroUpdate', { devicestatus: [] });
              } else {
                // process raw docs into runtime-ready objects
                try {
                  var processed = ctx.ddata.processRawDataForRuntime(docs);
                  socket.compress(true).emit('retroUpdate', { devicestatus: processed });
                } catch (procErr) {
            ...
            console.info('sent retroUpdate (devicestatus range)', opts);
            return;
          } catch (e) {
