        ...
        if (opts.devicestatus && (opts.from || opts.to)) {
          try {
            ...
            if (opts.from && opts.to) {
              query = { mills: { $gte: Number(opts.from), $lte: Number(opts.to) } };
            } else if (opts.from) {
              query = { mills: { $gte: Number(opts.from) } };
            } else if (opts.to) {
              query = { mills: { $lte: Number(opts.to) } };
            }

            ctx.store.collection(env.devicestatus_collection).find(query).sort({ mills: 1 }).toArray(function(err, docs) {
              if (err) {
