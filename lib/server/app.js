...
function create (env, ctx) {
  ...
  if (process.env.NODE_ENV !== 'development') {
    ...
    // Use a separate temp directory for minify cache to avoid conflicts with bundle source
    const minifyCacheDir = path.join(process.cwd(), '.cache/minify');
    try {
      fs.mkdirSync(minifyCacheDir, { recursive: true });
    } catch (err) {
      console.warn('Could not create minify cache directory:', err);
    }
