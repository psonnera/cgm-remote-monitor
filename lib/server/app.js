...
function resolvePath(filePath, createIfMissing = false) {
  ...
  // If we should create the directory and it doesn't exist, create it
  if (createIfMissing) {
    try {
      // Prefer creating relative to process.cwd() for relative paths
      const dirToCreate = path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath);
      fs.mkdirSync(dirToCreate, { recursive: true });
      return dirToCreate;
    } catch (err) {
      console.warn('Could not create directory', filePath, err);
    }
  }
...
function create (env, ctx) {
  ...
  // Compress responses. Registered before static-file serving so that static
  // assets are gzipped too (not just API responses).
  app.use(compression({
  ...
  let cacheBuster = process.env.NODE_ENV == 'development' ? 'developmentMode': crypto.randomBytes(8).toString('hex');
  ...
  // Bundles are built ahead of time by the postinstall/bundle script and
  ...
  // Production bundling
  const bundlePath = resolvePath('node_modules/.cache/_ns_cache/public', true);
  
  // Check if bundle files exist and log a warning if they don't
  let bundleDirExists = false;
  try {
    bundleDirExists = fs.existsSync(path.join(bundlePath, 'js'));
  } catch (err) {
    console.warn('Could not check bundle directory:', err.message);
  }
  
  if (!bundleDirExists) {
    console.warn('WARNING: Bundle files not found at', bundlePath);
    console.warn('Webpack bundles may not have been generated.');
    console.warn('The application may not work correctly without bundles.');
    console.warn('Try running: bun run bundle');
  } else {
    console.log('Bundle files found at', bundlePath);
  }
  
  const tmpFiles =  express.static(bundlePath, {
    maxAge: maxAge
  });

  // Serve the Brotli-precompressed (.br) bundle to clients that accept it.
  // Webpack emits bundle.*.js.br at build time; we stream it with the right
  // headers and let the compression middleware skip it (Content-Encoding set).
  // Anything not matched (no br support, other assets) falls through to static.
  app.use('/bundle', function serveBrotli (req, res, next) {
    if (req.method !== 'GET' && req.method !== 'HEAD') { return next(); }
    if (!req.path.endsWith('.js')) { return next(); }
    if (!(req.headers['accept-encoding'] || '').toLowerCase().includes('br')) { return next(); }

    const brFile = path.join(bundlePath, req.path + '.br');
    // Guard against path traversal escaping the bundle directory.
    if (!brFile.startsWith(bundlePath)) { return next(); }

    fs.stat(brFile, function onStat (err, st) {
  ...
  // NOTE: runtime minification (express-minify + cssmin) was removed. Webpack
  // already minifies the JS/CSS bundles at build time, and all responses are
  // gzip-compressed by the compression middleware registered earlier, so
  // per-request minification added an unmaintained dependency and CPU cost for
  // negligible byte savings.
