...
function storage (env, ctx) {
  ...
  // persist all records: batch insert the entire list, then call fn(err, result)
  function persist (docs, fn) {
    create(docs, fn);
  }
  ...
  function create (docs, fn) {
    ...
    docs.forEach(function(doc) {
      ...
      function completeOne (err, updateResults) {
      ...
      api().replaceOne(query, doc, { upsert: true })
        .then(function (updateResults) { completeOne(null, updateResults); })
        .catch(function (err) { completeOne(err); });
    });
  }
  ...
  function getEntry (id, fn) {
    ...
    api().findOne({ _id: objId }).then(function (entry) {
      fn(null, entry);
    }).catch(function (err) {
      fn(err);
    });
