...
function init (env, ctx) {
  ...
  authorization.resolve = async function resolve (data, callback) {
    ...
    // Only emit admin notification if there's a significant delay (multiple failures)
    // This prevents false alarms for expired tokens, temporary issues, etc.
    const delayAmount = shouldDelayRequest(data.ip);
    if (delayAmount && delayAmount > 10000) {
