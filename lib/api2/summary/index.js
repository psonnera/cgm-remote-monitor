...
function configure (env, ctx) {
  ...
  function processTreatments(treatments, profile, hours) {
    ...
    for (let i = 0; i < treatments.length; i++) {
        ...
        if (t.eventType === 'Temporary Target' || t.eventType === 'Temporary Target Cancel') {
          const target = {
            duration: (Number(t.duration) || 0) * 60,
            mills: t.mills
          };

          if (!isNaN(Number(t.targetTop))) {
            target.targetTop = Math.round(Number(t.targetTop));
          }

          if (!isNaN(Number(t.targetBottom))) {
            target.targetBottom = Math.round(Number(t.targetBottom));
          }

          rVal.targets.push(target);
          continue;
      }
