...
var ALL_STATUS_FIELDS = ['reservoir', 'battery', 'clock', 'device'];

function init (ctx) {
  var moment = ctx.moment;
  var translate = ctx.language.translate;
  var reverseTranslate = ctx.language.reverseTranslate;
  ...
  pump.updateVisualisation = function updateVisualisation (sbx) {
    ...
    // Use pump status for pill color: green=normal, blue=bolusing, yellow=suspended
    var pumpPillClass = result.pumpStatusClass || statusClass(result.level);
    
    sbx.pluginBase.updatePillText(pump, {
      value: values.join(' ')
      , info: info
      , label: translate('Pump')
      , pillClass: pumpPillClass
    });
  };
  ...
  function updateStatus(pump, result) {
    if (pump.status) {
      if (_.isEmpty(pump.status)) {
        result.pumpStatusClass = 'pump-disabled';
        return;
      }
      ...
      if (pump.status.bolusing) {
        ...
        if (pump.warnOnSuspend && pump.status.suspended) {
          ...
          result.message = 'Pump Suspended';
        }
      }
      
      // Normalize status from any language to English key
      // This handles when uploaders (e.g., AAPS) send status in their configured language
      var normalizedStatus = reverseTranslate(status);
      var normalizedLower = normalizedStatus.toLowerCase();
      
      // Set pill class based on pump status: green=normal, blue=bolusing, yellow=suspended
      if (normalizedLower === 'bolusing') {
        result.pumpStatusClass = 'pump-bolusing';
      } else if (normalizedLower === 'suspended') {
        result.pumpStatusClass = 'pump-suspended';
      } else {
        result.pumpStatusClass = 'pump-normal';
      }
    }
  }
