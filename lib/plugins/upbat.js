...
function init(ctx) {
  ...
  upbat.analyzeData = function analyzeData (sbx) {
    ...
    function analyzeStatus (status) {
      ...
      // Skip if no uploader status available
      if (!uploaderStatus) {
        return;
      }
    ...
    _.forEach(result.devices, function eachDevice (device) {
      ...
      if (!first) {
        return; // Skip device if no statuses
      }
      ...
      // Only process if recentLowest exists and has uploader data
      if (recentLowest && recentLowest.uploader) {
  ...
  upbat.updateVisualisation = function updateVisualisation (sbx) {
    ...
    // Handle case where prop might not be set (no devicestatus data)
    if (!prop || !prop.devices || _.values(prop.devices).length === 0) {
      sbx.pluginBase.updatePillText(upbat, {
        value: '-'
        , labelClass: 'icon-battery-100'
        , pillClass: ''
        , info: []
      });
      return;
    ...
    if (_.values(prop.devices).length > 1) {
      infos = _.map(_.values(prop.devices), function toInfo (device) {
        var info = {
          label: device.name
          , value: device.min ? device.min.display : '-'
        };
    ...
    sbx.pluginBase.updatePillText(upbat, {
      value: prop && prop.display ? prop.display : '-'
