...
function init(ctx) {
    ...
    `;
      ...
      `;
        ...
        if (exchangeRateInfo) {
            tableHtml += `<p><em>${client.translate('Currency conversion is activated via AI_LLM_EXCHANGERATE_API_CURRENCY - make sure AI_LLM_EXCHANGERATE_API_KEY is set. Exchange rate (USD to %1): %2', { params: [exchangeRateInfo.currency, exchangeRateInfo.rate.toFixed(4)] })}</em></p>`;
