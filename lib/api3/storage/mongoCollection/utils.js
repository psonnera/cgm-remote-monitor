...
const _ = require('lodash')
  ...
  , regexGuard = require('../../../server/regex-guard')
;

/**
 * Ensure a filter value is a scalar before it is embedded into a Mongo query,
 * preventing NoSQL operator injection (e.g. value = { $gt: '' } or { $where }).
 * @param {*} value
 * @returns {*} the value, when it is a string/number/boolean
 */
...
function parseFilter (filterDef, logicalOperator, onlyValid) {
  ...
  for (const itemDef of filterDef) {
    ...
    switch (itemDef.operator) {
      case 'eq':
        item = assertPrimitive(itemDef.value);
        break;

      case 'ne':
        item = { $ne: assertPrimitive(itemDef.value) };
        break;

      case 'gt':
        item = { $gt: assertPrimitive(itemDef.value) };
        break;

      case 'gte':
        item = { $gte: assertPrimitive(itemDef.value) };
        break;

      case 'lt':
        item = { $lt: assertPrimitive(itemDef.value) };
        break;

      case 'lte':
        item = { $lte: assertPrimitive(itemDef.value) };
        break;
      ...
      case 're':
        item = { $regex: regexGuard.assertSafe(itemDef.value.toString()) };
        break;
