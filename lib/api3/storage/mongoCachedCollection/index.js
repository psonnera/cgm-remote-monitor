...
function MongoCachedCollection (ctx, env, colName, baseStorage) {
  ...
  console.log("DEBUGBASAL, updateInCache:", JSON.stringify(doc, null, 2));

  if (doc && doc.isValid === false) {
      console.log("DEBUGBASAL, Invalid doc deleted");
      deleteInCache([doc._id])
    }
    else {
      console.log("DEBUGBASAL, doc pushed to cache");
  ...
  console.log("DEBUGBASAL, deleteInCache:", JSON.stringify(docs, null, 2));
    let changes
    if (_.isArray(docs)) {
      if (docs.length === 0) {
        return
