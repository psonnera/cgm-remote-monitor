  ...
  const fetchStatus = async () => {
    try {
      ...
      if (serverSettings.runtimeState !== 'loaded') {
        setLoadingMessage('Server is starting and still loading data, retrying load in 5 seconds');
...
client.load = function load (serverSettings, callback) {
  ...
  // Compact IOB/COB time-series cache for the main chart. Built once when data
  // arrives (the live tail from ddata.devicestatus + the retro window from the
  // lean `iobcob` payload) and only re-merged on arrival, never per redraw. The
  // renderer slices these sorted {mills, v} arrays by the visible domain, so
  // devicestatus is never re-parsed while dragging/scrolling.
  client.iobCobSeries = { iob: [], cob: [], fetchedAt: 0 };
  client._iobCobLive = { iob: [], cob: [] };   // from recent ddata.devicestatus
  client._iobCobRetro = { iob: [], cob: [] };  // from latest retro iobcob window

  function byMills (a, b) { return a.mills - b.mills; }

  // Parse a devicestatus array once into a sorted [{mills, v}] series using the
  // iob/cob plugin extractor (single source of OpenAPS/Loop/pump parsing truth).
  function extractSeriesFromDeviceStatus (arr, plugin, key) {
    if (!plugin || !plugin.fromDeviceStatus) return [];
    var out = [];
    (arr || []).forEach(function (s) {
      try {
        var d = plugin.fromDeviceStatus(s);
        if (d && d[key] !== undefined && d[key] !== null && d.mills) {
          var m = Number(d.mills);
          if (isFinite(m) && m > 0) out.push({ mills: m, v: Number(d[key]) });
        }
      } catch (e) { /* skip unparseable record */ }
    });
    return out.sort(byMills);
  }

  // Merge two sorted {mills, v} arrays, dedupe by mills (the second wins).
  function mergeSeriesArrays (a, b) {
    var byKey = {};
    var i;
    for (i = 0; i < a.length; i++) { if (a[i] && isFinite(a[i].mills)) byKey[a[i].mills] = a[i]; }
    for (i = 0; i < b.length; i++) { if (b[i] && isFinite(b[i].mills)) byKey[b[i].mills] = b[i]; }
    return Object.keys(byKey).map(function (k) { return byKey[k]; }).sort(byMills);
  }

  // Rebuild the merged series the renderer reads from the two source windows.
  client.rebuildIobCobSeries = function rebuildIobCobSeries () {
    client.iobCobSeries.iob = mergeSeriesArrays(client._iobCobLive.iob, client._iobCobRetro.iob);
    client.iobCobSeries.cob = mergeSeriesArrays(client._iobCobLive.cob, client._iobCobRetro.cob);
    client.iobCobSeries.fetchedAt = Date.now();
  };

  // Set the retro window from the compact server payload ([{mills, iob, cob}]).
  client.setRetroIobCobFromRows = function setRetroIobCobFromRows (rows) {
    var iobArr = [], cobArr = [];
    (rows || []).forEach(function (r) {
      var m = Number(r.mills);
      if (!isFinite(m) || m <= 0) return;
      if (r.iob !== undefined && r.iob !== null) iobArr.push({ mills: m, v: Number(r.iob) });
      if (r.cob !== undefined && r.cob !== null) cobArr.push({ mills: m, v: Number(r.cob) });
    });
    client._iobCobRetro.iob = iobArr.sort(byMills);
    client._iobCobRetro.cob = cobArr.sort(byMills);
    client.rebuildIobCobSeries();
  };

  // Fallback retro path: derive the window from full devicestatus docs (used when
  // the server has no backfilled `iobcob` rows yet).
  client.setRetroIobCobFromDeviceStatus = function setRetroIobCobFromDeviceStatus (devicestatusArray) {
    var iobPlugin = client.plugins && client.plugins('iob');
    var cobPlugin = client.plugins && client.plugins('cob');
    client._iobCobRetro.iob = extractSeriesFromDeviceStatus(devicestatusArray, iobPlugin, 'iob');
    client._iobCobRetro.cob = extractSeriesFromDeviceStatus(devicestatusArray, cobPlugin, 'cob');
    client.rebuildIobCobSeries();
  };

  // Request the lean IOB/COB series for the last 50h to seed the chart.
  client.requestLast48DeviceStatus = function requestLast48DeviceStatus() {
    ...
    socket.emit('loadRetro', { iobcob: true, from: from, to: to });
  };

  // Refresh the live-tail series from the current full devicestatus (parsed once).
  client.updateLast48FromDeviceStatus = function updateLast48FromDeviceStatus(devicestatusArray) {
    try {
      var iobPlugin = client.plugins && client.plugins('iob');
      var cobPlugin = client.plugins && client.plugins('cob');
      client._iobCobLive.iob = extractSeriesFromDeviceStatus(devicestatusArray, iobPlugin, 'iob');
      client._iobCobLive.cob = extractSeriesFromDeviceStatus(devicestatusArray, cobPlugin, 'cob');
      client.rebuildIobCobSeries();
    } catch (e) {
      console.error('updateLast48FromDeviceStatus failed', e);
    }
  };
  ...
  function brushed () {
    ...
    if (brushedRange) {
      ...
      // Update the main xScale domain to match the brush selection, extended for
      // forecast headroom - not the raw brushExtent, or this clobbers the
      // headroom-extended domain focusDragged/chart.scroll already applied,
      // flickering the "now" marker and prediction curve to the clipped edge on
      // every drag tick (this fires synchronously on every brush.move call).
      if (chart.xScale && typeof chart.xScale.domain === 'function') {
        chart.xScale.domain(chart.createAdjustedRange ? chart.createAdjustedRange() : brushExtent);
      }
    }
    ...
    function updateCurrentSGV (entry) {
      ...
      if (value === 9) {
        if (currentBG) currentBG.textContent = '';
      } else if (value < 39) {
        if (currentBG) currentBG.innerHTML = client.errorcodes.toDisplay(value);
      } else if (value < 40) {
        if (currentBG) currentBG.textContent = 'LOW';
      } else if (value > 400) {
        if (currentBG) currentBG.textContent = 'HIGH';
      } else {
        if (currentBG) currentBG.textContent = scaleBg(value);
      }
    ...
    function updatePlugins (time) {
      ...
      if (viewMenuEl && client.sbx && client.sbx.pluginBase && client.sbx.pluginBase.forecastInfos) {
        var forecastInfos = client.sbx.pluginBase.forecastInfos;
        var selectedForecasts = client.settings.showForecast.split(' ').filter(Boolean);
        forecastInfos.forEach(function(info) {
          var li = document.createElement('li');
          var label = document.createElement('label');
          var checkbox = document.createElement('input');
          checkbox.type = 'checkbox';
          checkbox.setAttribute('data-forecast-type', info.type);
          checkbox.checked = selectedForecasts.indexOf(info.type) > -1;
          label.appendChild(checkbox);
          label.appendChild(document.createTextNode(info.label));
          li.appendChild(label);
          viewMenuEl.appendChild(li);
        });
      }

      // Add event delegation for forecast menu changes (bind once)
      if (viewMenuEl && !viewMenuEl.dataset.forecastBound) {
        ...
        viewMenuEl.dataset.forecastBound = 'true';
      }
...
updateContainerClass('remove', 'loading', false);
      ...
      console.error('[brushed] Exception in brushed(), resetting flag:', e);
    } finally {
      brushing = false;
    }
  }
  ...
  function updateClock () {
    // Schedule the next tick FIRST so the chain never breaks even if code below throws
    ...
    try {
      ...
      // Check connection health - if no data received for 5+ minutes and socket appears connected,
      // re-authorize to ensure we're still in the DataReceivers room
      checkConnectionHealth();
    } catch (e) {
      console.error('[updateClock] Exception in updateClock():', e);
    }
  }

  function checkConnectionHealth () {
    var now = Date.now();
    var timeSinceLastData = now - (client.dataLastUpdated || 0);
    var dataStale = timeSinceLastData > times.mins(5).msecs;
    
    // Only check every 60 seconds to avoid spamming
    if (!client._lastConnectionCheck || (now - client._lastConnectionCheck) > times.mins(1).msecs) {
      client._lastConnectionCheck = now;
      
      if (dataStale && socket && socket.connected && !client.documentHidden) {
        console.info('[ConnectionHealth] No data received for', Math.round(timeSinceLastData / 1000), 'seconds, re-authorizing socket');
  ...
  (function watchVisibility () {
    ...
    document.addEventListener(visibilityChange, function visibilityChanged () {
      ...
      if (prevHidden && !client.documentHidden) {
        console.info('[Visibility] Tab became visible, checking connection state');
        
        // Check if socket appears disconnected or stale
        var socketStale = !client.socketConnected || 
          (client.lastSocketConnect && Date.now() - client.lastSocketConnect > times.mins(5).msecs);
        
        if (socketStale && socket && socket.connected) {
          // Socket reports connected but we think it's stale - re-authorize to be safe
          console.info('[Visibility] Re-authorizing socket after returning from background');
          client.authorizeSocket();
        } else if (!socket.connected) {
          // Socket is disconnected, it should auto-reconnect but log for debugging
          console.info('[Visibility] Socket disconnected, waiting for reconnect');
        }
        
        // Always refresh chart data when returning from background
        refreshChart(true);
  ...
  // Safety-net interval: if the setTimeout chain in updateClock ever dies
  // (e.g. due to browser throttling or an unforeseen error), this fires
  // every 60 seconds as a backup heartbeat to keep the UI alive.
  setInterval(function heartbeat () {
    try {
      ...
      console.error('[heartbeat] Exception in safety-net interval:', e);
    }
  }, times.mins(1).msecs);
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /* global io */
  console.log('[Client] client.load: Creating socket connection');
  ...
  client.loadRetroIfNeeded = function loadRetroIfNeeded () {
    ...
    if (timeSinceLoaded > times.mins(3).msecs || rangeChanged) {
      ...
      // Request only the lean IOB/COB series for the current brushed/focus range
      // so retro browsing moves tiny {mills, iob, cob} rows instead of full
      // device-status blobs. This means APS prediction curves (which need the
      // full devicestatus predBGs/predicted blob) are only available while the
      // scrolled window still overlaps the live 48h devicestatus tail held in
      // client.ddata.devicestatus (see ddata.recentDeviceStatus) - beyond that,
      // openaps/loop plugins correctly find no recent record and render no
      // forecast. That is a data-availability gap, not a bug in the chart
      // viewport (see chart.createAdjustedRange).
      try {
  ...
  socket.on('retroUpdate', function retroUpdate (retroData) {
    ...
    // Populate the retro window of the IOB/COB chart series. Prefer the compact
    // server payload; fall back to full devicestatus if this setup hasn't been
    // backfilled into the lean `iobcob` collection yet.
    var gotData = false;
    if (retroData && retroData.iobcob) {
      client.setRetroIobCobFromRows(retroData.iobcob);
      gotData = retroData.iobcob.length > 0;
    } else if (retroData && retroData.devicestatus) {
      client.setRetroIobCobFromDeviceStatus(retroData.devicestatus);
      gotData = retroData.devicestatus.length > 0;
    }
    // Clear in-flight flag even if retro response was empty
    if (client._last48RequestInFlight) {
      client._last48RequestInFlight = false;
    }
    // Redraw chart so the renderer can use the newly arrived IOB/COB series
    try {
      if (chart && gotData) {
        chart.update(false);
      }
  ...
  client.authorizeSocket = function authorizeSocket () {
    ...
    socket.emit(
      ...
      , function authCallback (data) {
        ...
        } else if (!callbackFired && callback) {
          callbackFired = true;
          callback();
        }
        // After successful authorization, seed the IOB/COB series if not yet fetched
        try {
          if (!client.iobCobSeries || !client.iobCobSeries.fetchedAt) {
            setTimeout(function() {
  ...
  socket.on('connect', function() {
    ...
    client.lastSocketConnect = Date.now();
    client.authorizeSocket();
  });
  ...
  socket.on('disconnect', function(reason) {
    ...
    client.lastDisconnectTime = Date.now();
  });
  ...
  socket.on('reconnect', function(attemptNumber) {
    ...
    client.socketConnected = true;
    client.lastSocketConnect = Date.now();
    // Re-authorize on reconnect to rejoin DataReceivers room
    client.authorizeSocket();
  ...
  socket.on('reconnect_failed', function() {
    console.error('[Socket] Reconnection failed - socket.io gave up');
    // At this point socket.io has exhausted reconnection attempts
    // User will need to refresh the page
  });
  ...
  alarmSocket.on('connect', function() {
    console.info('[AlarmSocket] Connected');
    client.subscribeForAlarms();
  });

  alarmSocket.on('disconnect', function(reason) {
    console.warn('[AlarmSocket] Disconnected:', reason);
  });

  alarmSocket.on('reconnect', function(attemptNumber) {
    console.info('[AlarmSocket] Reconnected after', attemptNumber, 'attempts');
    // Re-subscribe on reconnect
    client.subscribeForAlarms();
  });

  alarmSocket.on('error', function(error) {
    console.error('[AlarmSocket] Error:', error.message || error);
  });
  ...
  */
  ...
  function dataUpdate (received, headless) {
    ...
    // Refresh the live-tail of the IOB/COB chart series from the current
    // devicestatus (parsed once here, not on every redraw).
    try {
      if (client.ddata && client.ddata.devicestatus) {
        client.updateLast48FromDeviceStatus(client.ddata.devicestatus);
      }
    } catch (e) {
      console.error('Error updating IOB/COB live series from devicestatus in dataUpdate', e);
    }
