...
client.crashed = function crashed () {
  ...
  $('#loadingMessageText').html('It appears the server has crashed. Please go to Heroku and reboot the server.');
}
...
client.load = function load (serverSettings, callback) {
  ...
  $('#chartContainer').on('wheel', function(e) {
    ...
    wheelTimeout = setTimeout(function() {
      // Scroll up = shorter timespan (negative deltaY), scroll down = longer timespan (positive deltaY)
  ...
  $('#chartContainer').on('touchmove', function(e) {
    if (e.originalEvent.touches.length === 2 && touchStartDistance !== null) {
      e.preventDefault();
      var touch1 = e.originalEvent.touches[0];
      var touch2 = e.originalEvent.touches[1];
      var dx = touch2.clientX - touch1.clientX;
