...
client.load = function load (serverSettings, callback) {
  ...
  // store for last-48h device-status derived COB/IOB values
  client.last48 = {
    iob: [],
    cob: [],
    fetchedAt: 0
  ...
  // Request last-48h device-status via retro load to initialize client.last48
  client.requestLast48DeviceStatus = function requestLast48DeviceStatus() {
    ...
    var to = Date.now();
    var from = to - times.hours(48).msecs;
  ...
  client.updateLast48FromDeviceStatus = function updateLast48FromDeviceStatus(devicestatusArray) {
    try {
      ...
      console.error('updateLast48FromDeviceStatus error', e);
    }
  };
  ...
  client.afterAuth = function afterAuth (isAuthenticated) {
    ...
    // Helper function to toggle elements and their tooltips together
    function toggleWithTooltip(selector, condition, tooltipTitle) {
      var element = $(selector);
      if (condition) {
        element.show();
        // Set tooltip and initialize it with south gravity (below element) to prevent overflow
        if (tooltipTitle) {
          element.attr('original-title', tooltipTitle);
          element.tooltip({ gravity: 's', fade: true, opacity: 0.9 });
        }
      } else {
        element.hide();
        // Remove tooltip completely
        element.removeAttr('original-title');
        // Destroy tooltip instance if it exists
    ...
    $('#lockedToggle').click(client.hashauth.requestAuthentication);
    toggleWithTooltip('#lockedToggle', !treatmentCreateAllowed && client.settings.showPlugins.indexOf('careportal') > -1, 'Token needed for Care Portal');
    toggleWithTooltip('#treatmentDrawerToggle', treatmentCreateAllowed && client.settings.showPlugins.indexOf('careportal') > -1, 'Care Portal');
    toggleWithTooltip('#boluscalcDrawerToggle', treatmentCreateAllowed && client.settings.showPlugins.indexOf('boluscalc') > -1, 'Bolus Wizard');

    if (isAuthenticated) client.notifies.updateAdminNotifies();

    // Edit mode
    toggleWithTooltip('#editbutton', client.settings.editMode && treatmentUpdateAllowed, 'Edit Mode');
  ...
  socket.on('retroUpdate', function retroUpdate (retroData) {
    ...
    if (retroData && retroData.devicestatus) {
      ...
      console.info('client.last48 updated from retro, iob=', client.last48.iob.length, 'cob=', client.last48.cob.length);
    }
  ...
  */
  ...
  $('#agpButton').click(function(event) {
    event.preventDefault();
