...
client.load = function load (serverSettings, callback) {
  ...
  function brushed () {
    ...
    if (brushedRange) {
      ...
      // Update the main xScale domain to match the brush selection, extended
      // for forecast display; a raw brushExtent domain here would be the last
      // writer in retro mode and push cursor-anchored forecasts off-canvas
      if (chart.xScale && typeof chart.xScale.domain === 'function') {
        chart.xScale.domain(chart.createAdjustedRange());
      }
    }
