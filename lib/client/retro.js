...
retro.init = function init(serverSettings, callback) {
  ...
  function brushed() {
    ...
    function updatePlugins(time) {
      ...
      try {
        ...
        console.log('Plugin setProperties error:', err);
      }

      try {
        retro.plugins.updateVisualisations(retro.sbx);
      } catch (err) {
        console.log('Plugin updateVisualisations error:', err);
      }
    }
  ...
  function dataUpdate(received, headless) {
    ...
    if (!retro.chart) {
      ...
      if (retro.entries.length === 0) {
        console.warn('No entries available yet, skipping chart initialization');
        return;
      }
