...
retro.init = function init(serverSettings, callback) {
  ...
  if (retro.tooltip) retro.tooltip.remove();
  ...
  function brushed() {
    ...
    function updatePlugins(time) {
      ...
      try {
        ...
        console.error('plugins.setProperties error:', err);
      }

      try {
        retro.plugins.updateVisualisations(retro.sbx);
      } catch (err) {
        console.error('plugins.updateVisualisations error:', err);
      }
    }
  ...
  function prepareEntries() {
    ...
    retro.entries = temp1.concat(temp2);
  ...
  socket.on('dataUpdate', function(received) {
    if (!retro.ddata || !retro.ddata.inRetroMode) {
      ...
      // In retro mode, still process profile and treatment changes
      var needsRefresh = false;
      
      if (received.profiles) {
        // Clear the profile cache before loading new data to ensure fresh values
        if (retro.profilefunctions.clear) {
          retro.profilefunctions.clear();
        }
        retro.profilefunctions.loadData(received.profiles);
        needsRefresh = true;
      }
      
      // Update profile treatments if received
      if (received.profileTreatments || received.tempbasalTreatments || received.combobolusTreatments) {
        if (received.profileTreatments) {
          retro.ddata.profileTreatments = received.profileTreatments;
        }
        if (received.tempbasalTreatments) {
          retro.ddata.tempbasalTreatments = received.tempbasalTreatments;
        }
        if (received.combobolusTreatments) {
          retro.ddata.combobolusTreatments = received.combobolusTreatments;
        }
        retro.profilefunctions.updateTreatments(retro.ddata.profileTreatments, retro.ddata.tempbasalTreatments, retro.ddata.combobolusTreatments);
        needsRefresh = true;
      }
      
      // Refresh the display to reflect profile changes
      if (needsRefresh && retro.chart) {
        // Force a complete chart redraw to refresh all profile-related elements
        retro.chart.update(true);
        if (retro.brushed) {
          retro.brushed();
        }
  ...
  function updateClock() {
    ...
    clockTimer = setTimeout(updateClock, 15000);
  }
  ...
  retro.setupDatePicker = function setupDatePicker() {
    ...
    $('#retroMenu').off('click').on('click', function(e) {
      e.preventDefault();
    ...
    // Previous day button
    $('#prevDay').off('click').on('click', function(e) {
      e.preventDefault();
    ...
    // Next day button
    $('#nextDay').off('click').on('click', function(e) {
      e.preventDefault();
      var currentDate = retro.now ? new Date(retro.now) : new Date(dateInput.val() + 'T12:00:00');
...
var wheelTimeout;
$('#chartContainer').off('wheel').on('wheel', function(e) {
  e.preventDefault();
...
$('#chartContainer').off('touchstart').on('touchstart', function(e) {
...
if (chartContainer) {
  ...
  if (chartContainer._retroTouchMoveHandler) {
    chartContainer.removeEventListener('touchmove', chartContainer._retroTouchMoveHandler);
  }
  chartContainer._retroTouchMoveHandler = touchMoveHandler;
  chartContainer.addEventListener('touchmove', touchMoveHandler, { passive: false });
}

$('#chartContainer').off('touchend touchcancel').on('touchend touchcancel', function() {
