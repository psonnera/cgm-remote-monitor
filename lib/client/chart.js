...
function init (client, d3, $) {
  ...
  // Coalesce IOB/COB line redraws to at most one per animation frame. During a
  // drag/scroll the domain changes many times per second; the renderer reads the
  // live xScale domain when it fires, so collapsing to one rAF render keeps the
  // lines in sync without re-running the draw dozens of times per frame.
  var iobCobRedrawScheduled = false;
  var iobCobRedrawDomain = null;
  function drawIobCobNow () {
    if (client.settings.enable.indexOf('openaps') < 0) return;
    // Render against the domain that was live when this redraw was scheduled — the
    // same (forecast-extended) domain the BG dots were drawn with. scrollUpdate /
    // focusDragged call chart.brush.move AFTER scheduling, which fires brushed()
    // and resets chart.xScale to the non-extended brush extent before this rAF
    // fires; without pinning the domain the IOB/COB lines would render on a
    // narrower scale and appear shifted into the future relative to the BG dots.
    var restore = null;
    if (iobCobRedrawDomain && chart.xScale && typeof chart.xScale.domain === 'function') {
      restore = chart.xScale.domain();
      chart.xScale.domain(iobCobRedrawDomain);
    }
    try {
      if (renderer.drawIOB) renderer.drawIOB();
      if (renderer.drawCOB) renderer.drawCOB();
    } finally {
      if (restore) chart.xScale.domain(restore);
    }
  }
  function scheduleIobCobRedraw () {
    // Snapshot the domain now, while it still matches the BG dots, so the deferred
    // render is immune to a later brush.move mutating chart.xScale.
    if (chart.xScale && typeof chart.xScale.domain === 'function') {
      iobCobRedrawDomain = chart.xScale.domain();
    }
    if (iobCobRedrawScheduled) return;
    iobCobRedrawScheduled = true;
    var raf = (typeof window !== 'undefined' && window.requestAnimationFrame)
      ? window.requestAnimationFrame
      : function (cb) { return setTimeout(cb, 16); };
    raf(function () {
      iobCobRedrawScheduled = false;
      drawIobCobNow();
    });
  }
  ...
  var focusDragStartHeadroom = null;

  function focusDragStarted (event) {
    ...
    if (brushedRange) {
      ...
      // Preserve the forecast headroom that was already visible when the drag
      // started, so the plot width (and the now-line position) doesn't jitter
      // between "no headroom" mid-drag and "headroom" once brush.move fires.
      focusDragStartHeadroom = Math.max(0, chart.xScale.domain()[1].getTime()
        - focusDragStartBrushExtent[1].getTime());
    } else {
      focusDragStartBrushExtent = null;
      focusDragStartHeadroom = null;
    }
  }

  function focusDragged (event) {
    if (!focusDragStartBrushExtent) return;

    var dx = event.x - focusDragStartX;
    ...
    var headroom = focusDragStartHeadroom || 0;
    var domainRange = [newStart, new Date(newEnd.getTime() + headroom)];

    // Update xScale domain directly during drag for immediate visual feedback.
    // Keep the forecast headroom captured at drag start so the plot width (and
    // now-line position) stays stable through the gesture instead of snapping
    // once brush.move fires the brushed event.
    chart.xScale.domain(domainRange);
    chart.xScaleBasals.domain(domainRange);

    // Update the x-axis to show new time labels
    chart.focus.select('.x.axis').call(chart.xAxis);

    // Update renderer elements to match new time domain
    if (renderer.addFocusCircles) renderer.addFocusCircles();
    if (renderer.addBasals) renderer.addBasals(client);
    scheduleIobCobRedraw();
    ...
    chart.focus.select('.now-line')
      .attr('x1', chart.xScale(newEnd))
      .attr('x2', chart.xScale(newEnd));

    // Update brush position (this will trigger brush event which calls client.brushed())
    chart.theBrush.call(chart.brush.move, newRange.map(chart.xScale2));
  }
  ...
  // Always create drag-area and drag-overlay for focus drag
  chart.drag = chart.focus.append('g').attr('class', 'drag-area');
  chart.dragOverlay = chart.charts.append('rect')
  ...
  chart.createBrushedRange = function() {
    ...
    var isRetroMode = (client.ddata && client.ddata.inRetroMode) || chart.inRetroMode();

    // In retro mode (dedicated retro page or brush moved back), show up to 24 hours but clamp to actual data
    if (isRetroMode) {
      if (!brushedRange) {
        // No brush yet, use 24 hours ending at data extent end, but don't start before data begins
        var startTime = Math.max(dataExtent[1].getTime() - client.focusRangeMS, dataExtent[0].getTime());
        return [
          new Date(startTime),
          dataExtent[1]
        ];
      }
      // Force 24-hour window ending at brush position, but clamp both ends to data extent
      var endTime = Math.min(range[1].getTime(), dataExtent[1].getTime());
      var startTime = Math.max(endTime - client.focusRangeMS, dataExtent[0].getTime());
      return [
        new Date(startTime),
        new Date(endTime)
      ];
    ...
    // Live mode: if no brush, create one based on focusRangeMS
    if (!brushedRange) {
      range = dataExtent;
    ...
    // Live mode: apply focusRangeMS window
    var end = client.now > dataExtent[1].getTime() ? client.now : dataExtent[1].getTime();
  ...
  chart.createAdjustedRange = function() {
    ...
    // Extend the range to make room for the forecast curve, in both live and retro
    // mode. client.forecastTime is always derived from the brushed range's end (see
    // setForecastTime), never from real "now", so this never reaches past the brush
    // end by more than focusRangeMS even when scrolled back.
  ...
  chart.update = function update (init) {
    ...
    // Always set xScale2 domain to dataRange
    ...
    // Always enforce xScale domain to currentRange (retro or live)
    if (chart.xScale && typeof chart.xScale.domain === 'function') {
      chart.xScale.domain(currentRange);
    }
    if (chart.xScaleBasals && typeof chart.xScaleBasals.domain === 'function') {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        // Set initial xScale domain before rendering axis
        chart.xScale.domain(currentRange);
        chart.xScaleBasals.domain(currentRange);
        ...
        // Remove any existing drag-overlay before adding a new one
        chart.charts.selectAll('.drag-overlay').remove();
        // Make the drag-sensitive area extend from the bottom of the SVG up to the bottom of the focus area (x-axis/timescale)
        var dragStartY = focusHeight; // bottom of focus area (where x-axis is)
        var dragHeight = chartHeight + PADDING_BOTTOM - dragStartY;
        ...
        // Attach drag behavior only to the overlay below the focus area
        ...
        // Set xScale domain before redrawing
        chart.xScale.domain(currentRange);
        chart.xScaleBasals.domain(currentRange);
    ...
    // Always update drag overlay size and attach drag handler for focus drag (retro and normal mode)
    // Restrict drag overlay to the area below the focus area (x-axis/timescale)
    chart.charts.selectAll('.drag-overlay').remove();
    var dragStartY = chart.focusHeight;
    var dragHeight = (chart.prevChartHeight || chartHeight) + PADDING_BOTTOM - dragStartY;
    var dragOverlay = chart.charts.append('rect')
      .attr('class', 'drag-overlay')
      .attr('x', 0)
      .attr('y', dragStartY)
      ...
      .style('pointer-events', 'all')
      .style('cursor', 'grab');
    dragOverlay.raise().call(chart.focusDrag);
  ...
  function scrollUpdate () {
    ...
    // draw IOB/COB lines (insulin/carbs-on-board) from the cached series (throttled)
    scheduleIobCobRedraw();
