...
function init (client, d3, $) {
  ...
  function focusDragged (event) {
    ...
    chart.xScale.domain(newRange);
    chart.xScaleBasals.domain(newRange);
  ...
  chart.createAdjustedRange = function() {
    ...
    // In retro mode (dedicated retro page or brush moved back), don't extend range for forecast time
    if (!isRetroMode) {
