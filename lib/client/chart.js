...
function init (client, d3, $) {
  ...
  function focusDragged (event) {
    ...
    // extend the visual end (not the brush) so cursor-anchored forecasts stay visible
    var visualEnd = newEnd;
    if (client.sbx && chart.getForecastData().length > 0) {
      visualEnd = new Date(Math.max(newEnd.getTime(), client.forecastTime));
    }
    chart.xScale.domain([newStart, visualEnd]);
    chart.xScaleBasals.domain([newStart, visualEnd]);
  ...
  chart.createAdjustedRange = function() {
    ...
    } else if (client.sbx && chart.getForecastData().length > 0) {
      // Retro browsing: extend so forecasts anchored at the retro cursor stay visible.
      // The dedicated retro page has no forecast points, so it keeps the unextended range.
      adjustedRange[1] = new Date(Math.max(adjustedRange[1].getTime(), client.forecastTime));
    }
  ...
  chart.scroll = function scroll (nowDate) {
    // refresh forecastTime for the current brush position before computing the
    // adjusted range, otherwise the domain extension lags one brush move behind
    chart.setForecastTime();
    scrollNow = nowDate;
  ...
  chart.setForecastTime = function setForecastTime () {
    ...
    if (client.sbx && client.sbx.pluginBase.forecastPoints) {
