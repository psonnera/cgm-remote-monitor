  ...
  chart.update = function update (init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        // add a transparent overlay for drag interactions covering IOB/COB area and below
        // Attached to charts group (not basals) so it's always visible even when basals are hidden
        var iobCobAreaHeight = focusHeight * 0.3; // IOB/COB typically in lower 30% of focus
        var dragOverlay = chart.charts.append('rect')
          .attr('class', 'drag-overlay')
          .attr('x', 0)
          .attr('y', focusHeight - iobCobAreaHeight) // position in lower focus area
          .attr('width', chartWidth)
          .attr('height', iobCobAreaHeight + focusBottomMargin + basalsHeight + contextTopMargin + 50) // full height
