...
function init (client, $) {
  ...
  function gatherData () {
    ...
    var data = {
      ...
      , duration: (function() { 
        ...
        if (!durationInput) return durationInput;
        
        // First, try to parse as a plain number (most common case)
        var numericValue = parseFloat(durationInput);
        if (!isNaN(numericValue)) {
          return numericValue;
        }
        
        // Try to parse as a duration string (e.g., "30m", "1h") if parse_duration is available
        if (typeof parse_duration === 'function') {
          try {
            var parsed = parse_duration(durationInput);
            if (parsed) {
              return times.msecs(parsed).mins < 1 ? durationInput : times.msecs(parsed).mins;
            }
          } catch (e) {
  ...
  careportal.save = function save (event) {
    try {
      ...
      console.error('Error saving careportal entry:', err);
      window.alert(translate('Error') + ': ' + err.message);
    }
    maybePrevent(event);
  };
  ...
  function postTreatment (data) {
    function postSingleTreatment (treatmentData) {
      $.ajax({
        method: 'POST'
    ...
    function buildProfileSwitchBackTreatment (baseTreatment) {
      if (baseTreatment.eventType !== 'Profile Switch') {
        return null;
      }

      var durationMins = Number(baseTreatment.duration || 0);
      if (!durationMins || durationMins <= 0) {
        return null;
      }

      var startMills = baseTreatment.eventTime
        ? new Date(baseTreatment.eventTime).getTime()
        : new Date(baseTreatment.created_at).getTime();

      var fromProfile = client.profilefunctions.activeProfileToTime(startMills - 1);
      var targetProfile = baseTreatment.profile;

      if (fromProfile && client.profilefunctions.profileSwitchName) {
        fromProfile = client.profilefunctions.profileSwitchName(fromProfile);
      }
      if (targetProfile && client.profilefunctions.profileSwitchName) {
        targetProfile = client.profilefunctions.profileSwitchName(targetProfile);
      }

      if (!fromProfile || (targetProfile && fromProfile === targetProfile)) {
        return null;
      }

      return {
        enteredBy: baseTreatment.enteredBy
        , eventType: 'Profile Switch'
        , profile: fromProfile
        , autoSwitchBack: true
        , autoSwitchBackFromMills: startMills
        , created_at: new Date(startMills + times.mins(durationMins).msecs).toISOString()
        , notes: (baseTreatment.notes ? baseTreatment.notes + ' ' : '') + '[auto-switch-back from:' + startMills + ']'
        , units: baseTreatment.units
      };
    }

    var treatmentData = _.cloneDeep(data);

    if (treatmentData.eventType === 'Combo Bolus') {
      treatmentData.enteredinsulin = treatmentData.insulin;
      treatmentData.insulin = treatmentData.enteredinsulin * treatmentData.splitNow / 100;
      treatmentData.relative = treatmentData.enteredinsulin * treatmentData.splitExt / 100 / treatmentData.duration * 60;
    }

    var switchBackTreatment = buildProfileSwitchBackTreatment(treatmentData);

    postSingleTreatment(treatmentData);
    if (switchBackTreatment) {
      postSingleTreatment(switchBackTreatment);
    }

    storage.set('enteredBy', treatmentData.enteredBy);
  ...
  $('#treatmentSubmit').click(careportal.save);
  $('#eventTime').on('change', 'input:radio', careportal.eventTimeTypeChange);
