...
function init (client, d3) {
  ...
  renderer.drawIOB = function drawIOB () {
    try {
      ...
      // Filter points to visible chart domain (handles both real-time and retro-browsing)
      try {
        var xDomain = chart().xScale.domain();
        var domainStart = xDomain[0];
        var domainEnd = xDomain[1];
        
        // Find indices of points within domain
        var firstVisibleIdx = -1;
        var lastVisibleIdx = -1;
        
        for (var i = 0; i < points.length; i++) {
          if (points[i].d >= domainStart && points[i].d <= domainEnd) {
            if (firstVisibleIdx === -1) firstVisibleIdx = i;
            lastVisibleIdx = i;
          }
        }
        
        // Include one point before and after for smooth edge rendering
        if (firstVisibleIdx > 0) firstVisibleIdx--;
        if (lastVisibleIdx < points.length - 1) lastVisibleIdx++;
        
        var visiblePoints = [];
        if (firstVisibleIdx >= 0 && lastVisibleIdx >= 0) {
          visiblePoints = points.slice(firstVisibleIdx, lastVisibleIdx + 1);
        }
