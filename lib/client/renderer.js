...
function init (client, d3) {
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    ...
    _.forEach(profileTreatments, function eachTreatment (d) {
      var previousProfile = client.profilefunctions.activeProfileToTime(d.mills - 1);

      data.push(Object.assign({}, d, {
        cutting: previousProfile
      }));

      if (d.duration && !d.cuttedby) {
        var endMills = times.mins(d.duration).msecs + d.mills;
        var endProfile = client.profilefunctions.activeProfileToTime(endMills + 1);
        var hasExplicitEndSwitch = _.some(profileTreatments, function (t) {
          ...
          var tProfile = t.profile ? client.profilefunctions.profileSwitchName(t.profile) : '';
          var targetProfile = endProfile ? client.profilefunctions.profileSwitchName(endProfile) : '';
          return !!tProfile && !!targetProfile && tProfile === targetProfile;
        });

        // Only render end marker when profile actually changes at the end.
        if (endProfile && endProfile !== d.profile && !hasExplicitEndSwitch) {
          data.push({
            cutting: d.profile
            , profile: endProfile
            , mills: endMills
    ...
    var generateText = function(t) {
      var sign = t.first ? '▲▲▲' : '▬▬▬';
      var profileName = t.profile ? client.profilefunctions.profileSwitchName(t.profile).trim() : '';
      var cuttingName = t.cutting ? client.profilefunctions.profileSwitchName(t.cutting).trim() : '';
