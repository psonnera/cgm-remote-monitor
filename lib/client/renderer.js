...
function init (client, d3) {
  ...
  renderer.drawIOB = function drawIOB () {
    try {
      ...
      // map IOB values into a small vertical band inside the focus area
      ...
      var focusHeight = chart().focusHeight || (chart().prevChartHeight || 300) * 0.7;
      var topPadding = 8;
      var bottomPadding = 8;
      var bandHeight = Math.min( Math.max(40, Math.floor(focusHeight * 0.18)), Math.floor(focusHeight * 0.5) );
      var bandTop = topPadding;
      var bandBottom = bandTop + bandHeight;

      var iobYScale = d3.scaleLinear()
        .domain([minIOB, maxIOB])
        .range([bandBottom, bandTop]);
