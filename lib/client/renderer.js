...
function init (client, d3) {
  ...
  function getOrAddDate(entry) {
    ...
    // Treatments use created_at, SGV entries use mills
    var timestamp = entry.mills || entry.created_at || entry.date;
    if (!timestamp) return undefined;
    entry.date = new Date(timestamp);
    return entry.date;
  }
  ...
  renderer.shouldRenderTreatmentProfiles = function shouldRenderTreatmentProfiles () {
    var profileTreatments = (client.ddata && client.ddata.profileTreatments) || [];
    if (profileTreatments.length > 0) {
      return true;
    }

    return client.profilefunctions.listBasalProfiles().length >= 2;
  };
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    function updateTreatCircles (sel) {
      ...
      sel.attr('cx', function(d) {
        ...
        .attr('cy', function(d) {
          // Use same positioning logic as treatment bubbles
          var yValue = client.sbx.scaleEntry(d);
          if (isNaN(yValue) || yValue === undefined) {
            // Find nearest SGV entry
            var treatmentDate = getOrAddDate(d);
            if (treatmentDate && treatmentDate.getTime) {
              var treatmentTime = treatmentDate.getTime();
              var nearestEntry = null;
              var minTimeDiff = Infinity;
              
              for (var i = 0; i < client.entries.length; i++) {
                var entry = client.entries[i];
                if (entry.type === 'sgv' && entry.mgdl) {
                  var timeDiff = Math.abs(entry.mills - treatmentTime);
              ...
              if (nearestEntry && minTimeDiff < 15 * 60 * 1000) {
                yValue = utils.scaleMgdl(nearestEntry.mgdl);
              } else {
                yValue = utils.scaleMgdl((client.settings.thresholds.bgHigh + client.settings.thresholds.bgLow) / 2);
              }
            } else {
              yValue = utils.scaleMgdl((client.settings.thresholds.bgHigh + client.settings.thresholds.bgLow) / 2);
            }
          }
          return chart().yScale(yValue);
        })
        .attr('r', function() {
  ...
  function appendTreatments (treatment, arc) {
            ...
            if (window.confirm(translate('Change treatment time to %1 ?', { params: [newTime.toLocaleTimeString()] }))) {
              client.socket.emit(
                ...
                , function callback (result) {
                  ...
                  // If in retro mode, force chart and focus update
                  if (client.chart && client.brushed && typeof client.chart.inRetroMode === 'function' && client.chart.inRetroMode()) {
    ...
    // Calculate Y position - use BG value if available, otherwise find nearest SGV
    var treatmentY = client.sbx.scaleEntry(treatment);
    if (isNaN(treatmentY) || treatmentY === undefined) {
      // Find the nearest SGV entry to this treatment
      var treatmentDate = getOrAddDate(treatment);
      if (!treatmentDate || !treatmentDate.getTime) {
        // Invalid treatment date, position at middle of range
        treatmentY = utils.scaleMgdl((client.settings.thresholds.bgHigh + client.settings.thresholds.bgLow) / 2);
      } else {
        var treatmentTime = treatmentDate.getTime();
        var nearestEntry = null;
        var minTimeDiff = Infinity;
        ...
        if (nearestEntry && minTimeDiff < 15 * 60 * 1000) { // Within 15 minutes
          treatmentY = utils.scaleMgdl(nearestEntry.mgdl);
        } else {
          // No nearby SGV, position at middle of range
          treatmentY = utils.scaleMgdl((client.settings.thresholds.bgHigh + client.settings.thresholds.bgLow) / 2);
        }
      }
    }
    ...
    var treatmentDots = chart().focus.selectAll('treatment-insulincarbs')
      ...
      .attr('transform', 'translate(' + chart().xScale(getOrAddDate(treatment)) + ', ' + chart().yScale(treatmentY) + ')')
  ...
  renderer.addBasals = function addBasals (client) {
    ...
    // Draw grey overlay for periods when pump is disabled (pump.status is empty)
    var devicestatuses = (client.ddata && client.ddata.devicestatus) ? client.ddata.devicestatus : [];
    
    // Merge in retro-loaded device-status for complete history
    ...
    if (retroStatuses && retroStatuses.length) {
      devicestatuses = _.uniqBy((devicestatuses || []).concat(retroStatuses), function(s) { return s && (s._id || s.mills); });
    }
    
    // Find pump disabled periods (where pump.status is empty/falsy)
    var disabledPeriods = [];
    var currentDisabledStart = null;
    
    // Sort by mills to ensure chronological order
    var sortedStatuses = _.sortBy(devicestatuses, function(s) { return s.mills; });
    
    _.forEach(sortedStatuses, function(status) {
      var mills = status.mills;
      var isPumpDisabled = status.pump && _.isEmpty(status.pump.status);
      
      if (isPumpDisabled && !currentDisabledStart) {
        // Pump became disabled
        currentDisabledStart = mills;
      } else if (!isPumpDisabled && currentDisabledStart) {
        // Pump resumed
        disabledPeriods.push({ start: currentDisabledStart, end: mills });
        currentDisabledStart = null;
      }
    });
    
    // If pump is still disabled at end of data, close the period at current time
    if (currentDisabledStart) {
      disabledPeriods.push({ start: currentDisabledStart, end: client.sbx.time });
    }
    
    // Draw grey rectangles for disabled periods
    _.forEach(disabledPeriods, function(period) {
      if (period.start < to && period.end > from) {
        var x0 = chart().xScaleBasals(Math.max(period.start, from));
        var x1 = chart().xScaleBasals(Math.min(period.end, to));
        
        g.append('rect')
          .attr('class', 'pump-disabled-area')
          .attr('x', x0)
          .attr('y', 0)
          .attr('width', x1 - x0)
          .attr('height', chart().basalsHeight)
          .attr('fill', '#808080')
  ...
  // First index i where series[i].mills >= target (series sorted ascending by mills).
  function seriesLowerBound (series, target) {
    var lo = 0, hi = series.length;
    while (lo < hi) {
      var mid = (lo + hi) >> 1;
      if (series[mid].mills < target) lo = mid + 1; else hi = mid;
    }
    return lo;
  }

  // Slice a pre-sorted {mills, v} series to the visible x-domain (plus one point on
  // each side for edge continuity) via binary search, returning [{d, v, mills}].
  // This is the per-redraw hot path for the IOB/COB lines: O(log n + visible) and
  // touches no devicestatus, so dragging/scrolling stays smooth.
  function visibleSeriesSlice (series) {
    if (!series || !series.length) return [];
    var dom = chart().xScale.domain();
    var startMs = (dom[0] && dom[0].getTime) ? dom[0].getTime() : Number(dom[0]);
    var endMs = (dom[1] && dom[1].getTime) ? dom[1].getTime() : Number(dom[1]);
    if (!isFinite(startMs) || !isFinite(endMs)) {
      startMs = series[0].mills;
      endMs = series[series.length - 1].mills;
    }
    var firstIdx = seriesLowerBound(series, startMs);
    var lastIdx = seriesLowerBound(series, endMs + 1) - 1;
    var lo = Math.max(0, firstIdx - 1);
    var hi = Math.min(series.length - 1, lastIdx + 1);
    if (hi < lo) hi = lo;
    var out = [];
    for (var i = lo; i <= hi; i++) {
      out.push({ d: new Date(series[i].mills), v: Number(series[i].v), mills: series[i].mills });
    }
    return out;
  }

  // Above this many visible points we stop drawing per-point dots (the line shape
  // is unchanged; this just avoids a DOM-node explosion on wide retro windows).
  var IOBCOB_DOT_CAP = 200;
  ...
  renderer.drawIOB = function drawIOB () {
    try {
      ...
      // Read the precomputed, cached IOB series (built once when data arrives) and
      // slice it to the visible domain via binary search. No devicestatus is
      // re-parsed here, so redraws while dragging/scrolling stay cheap.
      chart().focus.selectAll('.iob-line').remove();
      chart().focus.selectAll('.iob-dot').remove();
      var points = visibleSeriesSlice(client.iobCobSeries && client.iobCobSeries.iob);
      ...
      // Create segments: split the line at zero crossings so negative and positive sections
      // can be colored differently (yellow for negative, cyan for positive/zero).
      // Also split at large time gaps so that periods with no device-status data are
      // left blank (unknown) instead of being bridged by a straight line.
      var MAX_GAP_MS = 15 * 60 * 1000; // gaps larger than this are treated as missing data
      var segments = [];
      var currentSegment = [];

      // close the in-progress segment, deriving its color from its first point
      function closeSegment () {
        if (currentSegment.length > 0) {
          var firstVal = Number(currentSegment[0].v);
          segments.push({
            points: currentSegment,
            color: (firstVal < 0) ? 'yellow' : 'cyan'
          });
          currentSegment = [];
        }
      }
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        // If there's a large gap from the previous point, end the current segment
        // so no line is drawn across the period with no data.
        if (currentSegment.length > 0) {
          var prevPoint = currentSegment[currentSegment.length - 1];
          if (p.mills - prevPoint.mills > MAX_GAP_MS) {
            closeSegment();
          }
        }
        ...
        // Check if we cross zero between this point and the next
        if (i < points.length - 1) {
          var nextP = points[i + 1];
          var currVal = Number(p.v);
          var nextVal = Number(nextP.v);

          // Don't interpolate a zero crossing across a data gap
          var gapToNext = (nextP.mills - p.mills) > MAX_GAP_MS;

          // If sign changes, we need to split the segment
          if (!gapToNext && ((currVal < 0 && nextVal >= 0) || (currVal >= 0 && nextVal < 0))) {
            // Add interpolated zero-crossing point
            var fraction = Math.abs(currVal) / (Math.abs(currVal) + Math.abs(nextVal));
            var crossTime = p.mills + fraction * (nextP.mills - p.mills);
            var crossPoint = { d: new Date(crossTime), v: 0, mills: crossTime };
            currentSegment.push(crossPoint);

            // Save current segment
            segments.push({
              points: currentSegment,
              color: (currVal < 0) ? 'yellow' : 'cyan'
            });

            // Start new segment with the zero-crossing point
            currentSegment = [crossPoint];
          }
        }
      }
      ...
      if (points.length <= IOBCOB_DOT_CAP) g.selectAll('circle.iob-dot')
  ...
  renderer.drawCOB = function drawCOB () {
    try {
      ...
      // Read the precomputed, cached COB series (built once when data arrives) and
      // slice it to the visible domain via binary search. No devicestatus is
      // re-parsed here, so redraws while dragging/scrolling stay cheap.
      chart().focus.selectAll('.cob-line').remove();
      chart().focus.selectAll('.cob-dot').remove();
      var points = visibleSeriesSlice(client.iobCobSeries && client.iobCobSeries.cob);
      if (!points || points.length === 0) {
        return;
      }
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        // If there's a large gap from the previous point, end the current segment
        // so no line is drawn across the period with no data.
        var gapBefore = i > 0 && (p.mills - points[i-1].mills > MAX_GAP_MS);
        if (gapBefore && currentSegment.length > 0) {
        ...
        if (pVal !== 0) {
          ...
          if (currentSegment.length === 0 && i > 0 && !gapBefore && Number(points[i-1].v) === 0) {
      ...
      if (points.length <= IOBCOB_DOT_CAP) g.selectAll('circle.cob-dot')
        .data(visiblePoints)
        .enter()
        .append('circle')
        .attr('class', 'cob-dot')
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    // Keep rendering profile switches even when the base profile store only has one
    // named profile; AAPS percentage/temp switches can still exist in treatments.
    if (!renderer.shouldRenderTreatmentProfiles()) {
      chart().basals.selectAll('.g-profile').remove();
      return;
    }
    ...
    var mode = client.settings.extendedSettings.basal.render;
    var profileTreatments = (client.ddata.profileTreatments || []).filter(function filterSuppressedTreatments (t) {
      return !(client.profilefunctions.isSuppressedProfileTreatment && client.profilefunctions.isSuppressedProfileTreatment(t));
    });
    var data = [];
    var getProfileStateAtTime = client.profilefunctions.profileSwitchStateAtTime
      ? client.profilefunctions.profileSwitchStateAtTime
      : function (mills) {
        return { profile: client.profilefunctions.activeProfileToTime(mills) };
      };
    var getProfileStateKey = client.profilefunctions.profileSwitchStateKey
      ? client.profilefunctions.profileSwitchStateKey
      : function (treatment) {
        return treatment && treatment.profile ? client.profilefunctions.profileSwitchName(treatment.profile) : '';
      };
    var getProfileDisplay = client.profilefunctions.profileSwitchDisplay
      ? client.profilefunctions.profileSwitchDisplay
      : function (treatment) {
        return treatment && treatment.profile ? client.profilefunctions.profileSwitchName(treatment.profile) : '';
      };

    // Add current active profile marker near the left side of the visible range.
    data.push(Object.assign({}, getProfileStateAtTime(from), {
      mills: from
      , first: true
      , profileLabel: getProfileDisplay(getProfileStateAtTime(from))
    }));

    _.forEach(profileTreatments, function eachTreatment (d) {
      var previousState = getProfileStateAtTime(d.mills - 1);
      var currentStateKey = getProfileStateKey(d);

      data.push(Object.assign({}, d, {
        cutting: previousState.profile
        , cuttingLabel: getProfileDisplay(previousState)
        , profileLabel: getProfileDisplay(d)
      }));

      if (d.duration && !d.cuttedby) {
        var endMills = times.mins(d.duration).msecs + d.mills;
        var endState = getProfileStateAtTime(endMills + 1);
        var endStateKey = getProfileStateKey(endState);
        var hasExplicitEndSwitch = _.some(profileTreatments, function (t) {
          if (t === d) {
            return false;
          }

          var sameTime = Math.abs(Number(t.mills) - Number(endMills)) <= times.mins(1).msecs;
          if (!sameTime) {
            return false;
          }

          var targetStateKey = getProfileStateKey(t);
          return !!targetStateKey && !!endStateKey && targetStateKey === endStateKey;
        });

        // Only render end marker when profile actually changes at the end.
        if (endState.profile && endStateKey !== currentStateKey && !hasExplicitEndSwitch) {
          data.push(Object.assign({}, endState, {
            cutting: d.profile
            , cuttingLabel: getProfileDisplay(d)
            , profileLabel: getProfileDisplay(endState)
            , mills: endMills
            , end: true
          }));
        }
    ...
    // Use a key function to ensure D3 properly matches elements to data
    // This prevents duplicate rendering when profile switches are added/removed
    var treatProfiles = chart().basals.selectAll('.g-profile').data(data, function(d) {
      // Create unique key using mills and profile type (first/end/regular)
      return d.mills + '-' + (d.first ? 'first' : d.end ? 'end' : 'switch');
    });
    ...
    var generateText = function(t) {
      var sign = t.first ? '▲▲▲' : '▬▬▬';
      var profileName = (t.profileLabel || getProfileDisplay(t) || '').trim();
      var cuttingName = (t.cuttingLabel || (t.cutting ? client.profilefunctions.profileSwitchName(t.cutting) : '') || '').trim();
      
      var ret;
      if (t.cutting && cuttingName && profileName && cuttingName !== profileName) {
