...
function init (client, d3) {
  ...
  renderer.drawCOB = function drawCOB () {
    try {
      ...
      // Create segments: split into continuous non-zero segments, adding vertical drops/rises at zero boundaries
      var segments = [];
      var currentSegment = [];
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        if (pVal !== 0) {
          // If this is the first non-zero point after zeros, add a vertical rise from zero
          ...
          if (currentSegment.length > 0) {
            // Add vertical drop to zero at the same time as last non-zero point
            var lastPt = currentSegment[currentSegment.length - 1];
            currentSegment.push({ d: lastPt.d, v: 0, mills: lastPt.mills });
      ...
      // draw small markers (skip if COB=0, except first zero after non-zero)
      var visiblePoints = points.filter(function(p, i) {
        var currentValue = Number(p.v);
        if (currentValue !== 0) {
          return true; // Always show non-zero points
        }
        // Show zero points only if previous point was non-zero (first zero after decline)
