...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    // Add icons for Temporary Target based on reason
    newRects.filter(function(d) {
      return d.eventType === 'Temporary Target' && d.reason;
    }).each(function(d) {
      var g = d3.select(this);
      
      // Determine emoji and color based on reason
      var emoji = '';
      var iconColor = 'white';
      
      if (d.reason === 'Activity' || d.reason === 'Physical Activity') {
        emoji = '🏃';
        iconColor = 'white';
      } else if (d.reason === 'Eating Soon') {
        emoji = '🍽️';
        iconColor = 'white';
      } else if (d.reason === 'Hypo') {
        emoji = '📉';
        iconColor = 'red';
      }
      
      if (emoji) {
        // Add black background circle
        g.append('circle')
          .attr('class', 'temp-target-icon-bg')
          .attr('cx', function() {
            if (d.durationType === "indefinite") {
              return 0;
            } else {
              return (chart().xScale(new Date(d.mills + times.mins(d.duration).msecs)) - chart().xScale(getOrAddDate(d))) / 2;
            }
          })
          .attr('cy', -15)
          .attr('r', 12)
        ...
        g.append('text')
          ...
          .attr('transform', function() {
            if (d.durationType === "indefinite") {
              ...
              var x = (chart().xScale(new Date(d.mills + times.mins(d.duration).msecs)) - chart().xScale(getOrAddDate(d))) / 2;
