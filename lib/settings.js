...
function init () {
  ...
  function enableAndDisableFeatures (accessor, nameType) {
    ...
    // Map deprecated plugin names to their modern replacements for backward compatibility
    var pluginAliases = {
      'bridge': 'connect',
      'mmconnect': 'connect'
    };
    
    enable = _.map(enable, function(plugin) {
      if (pluginAliases[plugin]) {
        console.info('Plugin "' + plugin + '" has been deprecated and replaced by "' + pluginAliases[plugin] + '". Automatically enabling "' + pluginAliases[plugin] + '" instead.');
        return pluginAliases[plugin];
      }
      return plugin;
    });
    
    // Remove duplicates in case both old and new plugin names were specified
    enable = _.uniq(enable);
